...
def _is_allowed_command(args: list[str]) -> bool:
    """Return True when args invoke this interpreter on project main.py."""
    ...
    if args[0] != sys.executable:
        return False
    try:
        return Path(args[1]).resolve() == MAIN_PY.resolve()
    except OSError:
        return False
...
def _run_command(args: list[str]) -> tuple[str, str]:
    ...
    if not _is_allowed_command(args):
        return "Error", "Rejected command: only the Audora interpreter running main.py is allowed"
    try:
...
def run_action(
    ...
    if triggered == "btn-demo":
        if demo_value not in ALLOWED_DEMO_MODES:
            return "Error", "Rejected demo mode"
