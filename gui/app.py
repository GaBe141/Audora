...
def _is_allowlisted_command(args: list[str]) -> bool:
    """Return True when the subprocess payload is the current interpreter + main.py."""
    ...
    if args[0] != sys.executable:
        return False
    try:
        return Path(args[1]).resolve() == MAIN_PY
    except OSError:
        return False
...
def _run_command(args: list[str]) -> tuple[str, str]:
    ...
    if not _is_allowlisted_command(args):
...
def run_action(
    ...
    if triggered == "btn-demo":
        if demo_value not in ALLOWED_DEMO_MODES:
            return "Error", f"Demo mode is not allowlisted: {demo_value}"
