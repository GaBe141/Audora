...
def validate_demo_mode(demo_value: str) -> str:
    """Allow only known demo modes from the GUI selector."""
...
def save_settings(_n, slack_url, discord_url, webhook_url, smtp_host, smtp_port, smtp_user, smtp_pass):
    try:
        ...
        if slack_url:
            slack_url = svc._validate_webhook_url(slack_url)
            svc.config["slack"]["webhook_url"] = slack_url
        if discord_url:
            discord_url = svc._validate_webhook_url(discord_url)
            svc.config["discord"]["webhook_url"] = discord_url
        if webhook_url:
            webhook_url = svc._validate_webhook_url(webhook_url)
...
def _test_channel_callback(channel_key: str, url_input_id: str, channel_enum_name: str):
    ...
    def _cb(_n, url):
        ...
        try:
            svc = EnhancedNotificationService()
            url = svc._validate_webhook_url(url)
