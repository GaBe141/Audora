...
def _run_command(args: list[str]) -> tuple[str, str]:
    """Run a tightly scoped command in subprocess; return (status, output)."""
    if not isinstance(args, list) or not all(isinstance(arg, str) for arg in args):
        return "Error", "Invalid command payload"
    expected_main = str(PROJECT_ROOT / "main.py")
    if len(args) < 2 or args[0] != sys.executable or args[1] != expected_main:
        return "Error", "Command must invoke the current interpreter and main.py"
    try:
...
def run_action(
    ...
    if triggered == "btn-demo":
        ...
        return _run_command(
            [sys.executable, str(PROJECT_ROOT / "main.py"), "--demo", demo_value]
        )
