...
def _validate_demo_mode(demo_value: str) -> str:
    """Return a known demo mode or raise ValueError."""
    if demo_value not in ALLOWED_DEMO_MODES:
        raise ValueError(f"Invalid demo mode: {demo_value}")
    return demo_value
...
def _test_channel_callback(channel_key: str, url_input_id: str, channel_enum_name: str):
    ...
    def _cb(_n, url):
        ...
        try:
            svc = EnhancedNotificationService()
            validated = svc._validate_webhook_url(url)
