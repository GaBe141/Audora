...
from dash import Input, Output, State, ctx, dash_table, dcc, html  # noqa: E402
...
ALLOWED_MAIN_TOKENS = frozenset(
    {"--mode", "single", "--demo", "--setup", "--validate", *ALLOWED_DEMO_MODES}
)
...
def _run_command(args: list[str]) -> tuple[str, str]:
    """Run an allowlisted main.py command; return (status_str, combined_stdout_stderr)."""
    ...
    if args[0] != sys.executable:
        return "Error", "Command interpreter is not allowed"
    if len(args) < 2 or Path(args[1]).resolve() != MAIN_PY.resolve():
        return "Error", "Command target is not allowed"
    extra = args[2:]
    if not extra or any(token not in ALLOWED_MAIN_TOKENS for token in extra):
        return "Error", "Command arguments are not allowed"
    try:
...
def run_action(
    ...
    if triggered == "btn-discovery":
        return _run_command([sys.executable, str(MAIN_PY), "--mode", "single"])
    if triggered == "btn-demo":
        if demo_value not in ALLOWED_DEMO_MODES:
            return "Error", "Invalid demo mode"
        return _run_command([sys.executable, str(MAIN_PY), "--demo", demo_value])
    if triggered == "btn-setup":
        return _run_command([sys.executable, str(MAIN_PY), "--setup"])
    if triggered == "btn-validate":
        return _run_command([sys.executable, str(MAIN_PY), "--validate"])
