...
def _run_command(args: list[str]) -> tuple[str, str]:
    """Run an allowlisted command in subprocess; return (status_str, output)."""
    ...
    if args[0] != sys.executable:
        return "Error", "Unapproved interpreter"
    if Path(args[1]).resolve() != MAIN_PY.resolve():
        return "Error", "Unapproved command"
    try:
...
def run_action(
    ...
    if triggered == "btn-demo":
        if demo_value not in ALLOWED_DEMO_MODES:
            return "Error", "Unapproved demo mode"
...
def _test_channel_callback(channel_key: str, url_input_id: str, channel_enum_name: str):
    ...
    def _cb(_n, url):
        ...
        try:
            ...
            validated_url = svc._validate_webhook_url(url, allow_private=allow_private)
            svc.config[channel_key][config_key] = validated_url
