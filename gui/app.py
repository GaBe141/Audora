...
def _validated_demo_mode(demo_value: str | None) -> str:
    """Return a demo mode only when it matches the GUI allowlist."""
    if demo_value not in ALLOWED_DEMO_MODES:
        raise ValueError(f"Invalid demo mode: {demo_value!r}")
    return demo_value
...
def _test_channel_callback(channel_key: str, url_input_id: str, channel_enum_name: str):
    ...
    def _cb(_n, url):
        ...
        try:
            svc = EnhancedNotificationService()
            svc.config[channel_key]["webhook_url" if channel_key != "webhook" else "url"] = (
                svc._validate_webhook_url(
                    url, allow_private=svc._allow_private_webhooks()
                )
            )
