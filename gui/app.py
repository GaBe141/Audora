...
def _run_command(args: list[str]) -> tuple[str, str]:
    ...
    expected_main = (PROJECT_ROOT / "main.py").resolve()
    try:
        requested_main = Path(args[1]).resolve()
    except (IndexError, OSError):
        return "Error", "Command target is not allowed"
    if requested_main != expected_main:
        return "Error", "Command target is not allowed"
...
def _test_channel_callback(channel_key: str, url_input_id: str, channel_enum_name: str):
    ...
    def _cb(_n, url):
        ...
        try:
            svc = EnhancedNotificationService()
            validated_url = svc._validate_webhook_url(url)
