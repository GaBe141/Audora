...
def _is_safe_argv(args: list[str]) -> bool:
    """Reject malformed subprocess argument lists."""
    return bool(args) and all(isinstance(part, str) and "\x00" not in part for part in args)


def _run_command(args: list[str]) -> tuple[str, str]:
    """Run a command in subprocess; return (status_str, combined_stdout_stderr)."""
    if not _is_safe_argv(args):
