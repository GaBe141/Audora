...
def _run_command(args: list[str]) -> tuple[str, str]:
    ...
    if args[0] != sys.executable:
        return "Error", "Commands must use the current Python interpreter"
    if len(args) < 2 or Path(args[1]).resolve() != _MAIN_PY.resolve():
        return "Error", "Commands may only invoke main.py"
...
def save_settings(_n, slack_url, discord_url, webhook_url, smtp_host, smtp_port, smtp_user, smtp_pass):
    try:
        ...
        if slack_url:
            slack_url = svc._validate_webhook_url(slack_url, allow_private=False)
            svc.config["slack"]["webhook_url"] = slack_url
        if discord_url:
            discord_url = svc._validate_webhook_url(discord_url, allow_private=False)
            svc.config["discord"]["webhook_url"] = discord_url
        if webhook_url:
            webhook_url = svc._validate_webhook_url(
...
def _test_channel_callback(channel_key: str, url_input_id: str, channel_enum_name: str):
    ...
    def _cb(_n, url):
        ...
        try:
            svc = EnhancedNotificationService()
            url_key = "webhook_url" if channel_key != "webhook" else "url"
            allow_private = (
                svc._allow_private_webhooks() if channel_key == "webhook" else False
            )
            svc.config[channel_key][url_key] = svc._validate_webhook_url(
