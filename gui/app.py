...
_ALLOWED_COMMAND_TOKENS = frozenset(
    {"--mode", "--demo", "--setup", "--validate", "single", *_ALLOWED_DEMO_MODES}
)
...
def _run_command(args: list[str]) -> tuple[str, str]:
    ...
    if args[0] != sys.executable or len(args) < 2 or args[1] != main_script:
        return "Error", "Command is not allowlisted"

    extra = args[2:]
    if any(token not in _ALLOWED_COMMAND_TOKENS for token in extra):
        return "Error", "Command is not allowlisted"
