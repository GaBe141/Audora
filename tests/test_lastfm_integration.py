...
class TestLastFmAPIErrorHandling:
    ...
    def test_api_error_in_json_raises_response_error(self):
        ...
        with patch.object(api.session, "get", return_value=mock_response), pytest.raises(
            APIResponseError, match="Invalid API key"
        ):
    ...
    def test_http_error_raises_connection_error(self):
        ...
        with patch.object(api.session, "get", return_value=mock_response), pytest.raises(
            APIConnectionError, match="429"
        ):
            api.get_top_artists_global(limit=5)


class TestLastFmTransportSecurity:
    """Last.fm API traffic must use HTTPS to protect API keys in transit."""
