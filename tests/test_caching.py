...
class TestRedisCacheSerialization:
    """Regression tests for safe Redis serialization without pickle loading."""
    ...
    def test_dataframe_round_trip(self):
        backend = object.__new__(RedisCacheBackend)
        value = pd.DataFrame({"track": ["A", "B"], "score": [91.5, 88.0]})
    ...
    def test_invalid_envelope_is_rejected(self):
        backend = object.__new__(RedisCacheBackend)
        payload = json.dumps({"v": 1, "type": "pickle", "value": "AAAA"}).encode()
