...
class TestRedisCacheSerialization:
    """Tests for Redis cache serialization without executable payloads."""

    def test_round_trips_json_compatible_values(self):
        backend = object.__new__(RedisCacheBackend)
        value = {"track": "Song", "scores": [1, 2.5, None], "active": True}
    ...
    def test_round_trips_dataframes(self):
        backend = object.__new__(RedisCacheBackend)
        value = pd.DataFrame({"track": ["Song A", "Song B"], "score": [95, 88]})
