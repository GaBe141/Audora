...
class TestUpdateTrendsBulk:
    ...
    def test_update_trends_bulk_rejects_invalid_update_fields(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        track_id = sample_trends[0].track_id
        ...
        with pytest.raises(ValueError, match="Invalid update field"):
            data_store.update_trends_bulk(
                [track_id],
                {"score = 1; DROP TABLE trends; --": 99.0},
            )

        # Ensure table still exists and original rows are intact.
        with data_store.get_connection() as conn:
            row = conn.execute("SELECT COUNT(*) FROM trends").fetchone()
            assert row is not None
            assert row[0] == len(sample_trends)

    def test_update_trends_bulk_serializes_metadata_dict(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        track_id = sample_trends[0].track_id
        updated = data_store.update_trends_bulk([track_id], {"metadata": {"source": "updated"}})

        assert updated >= 1
        with data_store.get_connection() as conn:
            row = conn.execute("SELECT metadata FROM trends WHERE track_id = ?", (track_id,)).fetchone()
            assert row is not None
            assert '"source": "updated"' in row[0]
