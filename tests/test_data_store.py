...
class TestUpdateTrendsBulk:
    ...
    def test_update_trends_bulk_rejects_invalid_field(self, data_store, sample_trends):
        ...
        with pytest.raises(ValueError, match="Invalid update fields"):
            data_store.update_trends_bulk([track_id], {"score = 0 WHERE 1=1 --": 99})

    def test_update_trends_bulk_serializes_metadata(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        track_id = sample_trends[0].track_id
        updates = {"metadata": {"source": "security-test", "safe": True}}
        updated = data_store.update_trends_bulk([track_id], updates)
        assert updated >= 1
        ...
        with data_store.get_connection() as conn:
            ...
            assert row is not None
            assert json.loads(row[0]) == updates["metadata"]
