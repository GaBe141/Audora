...
class TestUpdateTrendsBulk:
    ...
    def test_update_trends_bulk_invalid_field_raises_value_error(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        track_id = sample_trends[0].track_id
        with pytest.raises(ValueError, match="Invalid update fields"):
            data_store.update_trends_bulk([track_id], {"score": 50, "score = 0; DROP TABLE trends; --": 1})
