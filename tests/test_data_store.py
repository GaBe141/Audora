...
class TestExportToCsvSecurity:
    """Security tests for export_to_csv path handling."""

    def test_export_to_csv_blocks_absolute_paths(self, data_store):
        with pytest.raises(ValueError, match="must be a relative path"):
            data_store.export_to_csv("trends", "/tmp/export.csv")

    def test_export_to_csv_blocks_path_traversal(self, data_store):
        with pytest.raises(ValueError, match="must stay within exports directory"):
            data_store.export_to_csv("trends", "../outside.csv")

    def test_export_to_csv_writes_within_exports(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        relative_output = "safe/test_export.csv"
        output = data_store.export_to_csv("trends", relative_output)
        expected = (data_store.export_dir / relative_output).resolve()
