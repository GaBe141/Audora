...
class TestUpdateTrendsBulk:
    ...
    def test_update_trends_bulk_rejects_untrusted_field_names(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        target_id = sample_trends[0].track_id

        with pytest.raises(ValueError, match="Invalid update fields"):
            data_store.update_trends_bulk([target_id], {"score = 0; DROP TABLE trends; --": 99})
