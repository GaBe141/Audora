...
class TestUpdateTrendsBulk:
    ...
    def test_update_trends_bulk_rejects_invalid_field_names(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        with pytest.raises(ValueError, match="Invalid update fields"):
            data_store.update_trends_bulk([sample_trends[0].track_id], {"score = 0 --": 1})


class TestExportToCsvSecurity:
    """Test export_to_csv path safety checks."""
