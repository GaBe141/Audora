...
class TestUpdateTrendsBulk:
    ...
    def test_update_trends_bulk_serializes_metadata_mapping(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        track_id = sample_trends[0].track_id
        updated = data_store.update_trends_bulk([track_id], {"metadata": {"source": "unit"}})
        assert updated >= 1
        with data_store.get_connection() as conn:
            row = conn.execute(
