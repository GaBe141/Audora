...
class TestUpdateTrendsBulk:
    ...
    def test_update_trends_bulk_rejects_invalid_columns(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        track_id = sample_trends[0].track_id

        with pytest.raises(ValueError, match="Invalid update fields"):
            data_store.update_trends_bulk([track_id], {"score = 0 WHERE 1=1 --": 1})

    def test_update_trends_bulk_serializes_metadata_dict(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        track_id = sample_trends[0].track_id
        updated = data_store.update_trends_bulk([track_id], {"metadata": {"source": "security-test"}})
        ...
        with data_store.get_connection() as conn:
            ...
            assert parsed["source"] == "security-test"
