...
class TestUpdateTrendsBulk:
    ...
    def test_update_trends_bulk_rejects_invalid_fields(self, data_store, sample_trends):
        ...
        with pytest.raises(ValueError, match="Invalid update fields"):
            data_store.update_trends_bulk([track_id], {"score = 0 WHERE 1=1 --": 10})

    def test_update_trends_bulk_normalizes_metadata_dict(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        track_id = sample_trends[0].track_id
        metadata_update = {"source": "test", "count": 2}

        updated = data_store.update_trends_bulk([track_id], {"metadata": metadata_update})
        assert updated >= 1
        ...
        with data_store.get_connection() as conn:
            ...
            assert row is not None
            assert json.loads(row[0]) == metadata_update
