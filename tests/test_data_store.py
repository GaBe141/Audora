...
class TestUpdateTrendsBulk:
    ...
    def test_update_trends_bulk_rejects_invalid_update_field(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        track_id = sample_trends[0].track_id
        with pytest.raises(ValueError, match="Invalid update fields"):
            data_store.update_trends_bulk([track_id], {"score = 0, is_active = 0 --": 1})

    def test_update_trends_bulk_rejects_non_string_track_id(self, data_store, sample_trends):
        data_store.save_trends_bulk(sample_trends)
        with pytest.raises(TypeError, match="track_ids must be strings"):
            data_store.update_trends_bulk([123], {"score": 90.0})  # type: ignore[list-item]
