...
class _MockClientSession:
    def __init__(self, recorder, status=200):
        self.post = _MockPostContext(recorder, status)
...
class TestNotificationTransportSecurity:
    """Validate outbound transport security settings."""
    ...
    def test_webhook_channels_disable_redirects(self, channel, config_key):
        svc = EnhancedNotificationService()
        ...
        with (
            ...
            patch(
                ...
                side_effect=lambda: _MockClientSession(posts),
            ),
        ):
    ...
    def test_rejects_smtp_credentials_without_tls(self):
        svc = EnhancedNotificationService()
        svc.config["email"].update(
            {
                "smtp_server": "smtp.example.com",
