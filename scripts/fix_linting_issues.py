...
def main() -> None:
    ...
    for package in packages:
        run_command([python_exec, "-m", "pip", "install", package], f"Installing {package}")

    # Step 2: Run Black formatter
    print_step(2, "Format Code with Black")
    run_command(
        [python_exec, "-m", "black", "core/", "scripts/", "--line-length", "100", "--quiet"],
    ...
    for file in files_to_fix:
        run_command(
            [python_exec, "-m", "ruff", "check", file, "--fix", "--unsafe-fixes"],
            f"Fixing {file}",
        )

    # Step 4: Run tests
    print_step(4, "Run Tests to Verify Changes")
    run_command([python_exec, "test_project.py"], "Running test suite")
