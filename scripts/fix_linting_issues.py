...
def run_command(cmd: list[str], description: str) -> bool:
    """Run a command argv list and return success status."""
    print(f"  → {description}")
    if not isinstance(cmd, list) or not cmd or not all(isinstance(part, str) for part in cmd):
        print("  ❌ Failed: command must be a non-empty list of strings")
        return False
    try:
...
def main() -> None:
    ...
    for package in packages:
        run_command([python, "-m", "pip", "install", package], f"Installing {package}")
    ...
    for file in files_to_fix:
        run_command(
            [python, "-m", "ruff", "check", file, "--fix", "--unsafe-fixes"],
            f"Fixing {file}",
        )
