...
def main() -> None:
    ...
    for package in packages:
        run_command(shlex.split(f"pip install {package}"), f"Installing {package}")

    # Step 2: Run Black formatter
    print_step(2, "Format Code with Black")
    run_command(
        shlex.split("python -m black core/ scripts/ --line-length 100 --quiet"),
    ...
    for file in files_to_fix:
        run_command(
            shlex.split(f"python -m ruff check {file} --fix --unsafe-fixes"), f"Fixing {file}"
        )
