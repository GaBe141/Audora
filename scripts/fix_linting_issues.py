...
def run_command(cmd: list[str] | str, description: str) -> bool:
    ...
    Accepts either a list of argv tokens (preferred) or a shell-like string.
    Shell execution is explicitly disabled to avoid command injection.
    """
    print(f"  → {description}")
...
def main() -> None:
    ...
    run_command(
        ["python", "-m", "black", "core/", "scripts/", "--line-length", "100", "--quiet"],
    ...
    for file in files_to_fix:
        run_command(
            ["python", "-m", "ruff", "check", file, "--fix", "--unsafe-fixes"], f"Fixing {file}"
        )
