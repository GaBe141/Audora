...
class EnhancedMusicDataStore:
    ...
    def get_tracks_with_artists_bulk(
        ...
        # Create a deterministic cache key to avoid process-randomized hash()
        # behavior and to keep caching stable across interpreter restarts.
        normalized_pairs = sorted((str(track), str(artist)) for track, artist in track_artist_pairs)
        pairs_payload = json.dumps(normalized_pairs, separators=(",", ":"), ensure_ascii=True)
        cache_digest = hashlib.sha256(pairs_payload.encode("utf-8")).hexdigest()
