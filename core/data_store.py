...
class EnhancedMusicDataStore:
    ...
    def __init__(self, db_path: str = "enhanced_music_trends.db", backup_dir: str = "backups"):
        ...
        self._allowed_trend_update_fields = {
            "platform",
            "track_name",
            "artist",
            "score",
    ...
    def update_trends_bulk(self, track_ids: list[str], updates: dict[str, Any]) -> int:
        ...
        invalid_fields = sorted(field for field in updates if field not in self._allowed_trend_update_fields)
        if invalid_fields:
            raise ValueError(
                "Invalid update field(s): "
                f"{', '.join(invalid_fields)}. "
                f"Allowed fields: {', '.join(sorted(self._allowed_trend_update_fields))}"
            )
