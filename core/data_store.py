...
class EnhancedMusicDataStore:
    ...
    def get_tracks_with_artists_bulk(
        ...
        # Create a deterministic cache key (built-in hash is randomized per process)
        pairs_str = json.dumps(sorted(track_artist_pairs), ensure_ascii=True)
        pairs_digest = hashlib.sha256(pairs_str.encode("utf-8")).hexdigest()
