...
class EnhancedMusicDataStore:
    ...
    def export_to_csv(self, table: str, filepath: str, days: int | None = None) -> str:
        ...
        """
        # Restrict table selection to known static query map.
        if table not in self._EXPORT_QUERIES_NO_DAYS:
            valid_tables = sorted(self._EXPORT_QUERIES_NO_DAYS)
            raise ValueError(f"Invalid table name: {table}. Must be one of {valid_tables}")

        with self.get_connection() as conn:
            if days:
                query = self._EXPORT_QUERIES_WITH_DAYS[table]
                df = pd.read_sql_query(query, conn, params=[f"-{days} days"])
            else:
                query = self._EXPORT_QUERIES_NO_DAYS[table]
