...
class EnhancedMusicDataStore:
    ...
    def update_trends_bulk(self, track_ids: list[str], updates: dict[str, Any]) -> int:
        ...
        if any(not isinstance(track_id, str) for track_id in track_ids):
            raise TypeError("All track_ids must be strings")
        ...
        if invalid_fields:
            invalid_sorted = ", ".join(sorted(invalid_fields))
            raise ValueError(f"Invalid update fields: {invalid_sorted}")

        # Build SET clause from validated column names only.
        normalized_updates = {field: updates[field] for field in sorted(updates)}
