...
class EnhancedMusicDataStore:
    ...
    def _resolve_export_path(self, filepath: str) -> Path:
        """Confine CSV exports to the working tree, backup dir, or database directory."""
    ...
    def export_to_csv(self, table: str, filepath: str, days: int | None = None) -> str:
        ...
        Export paths are confined to the working tree, backup directory, or DB parent.
        """
        ...
        with self.get_connection() as conn:
            if days:
                ...
                LIMIT ?
                """
                df = pd.read_sql_query(query, conn, params=[f"-{days} days", row_cap + 1])
            else:
                # Table name is validated above, safe to use in query
                query = f"SELECT * FROM {table} ORDER BY created_at DESC LIMIT ?"
                df = pd.read_sql_query(query, conn, params=[row_cap + 1])

            if len(df) > row_cap:
                df = df.iloc[:row_cap]
                self.logger.warning(
                    "CSV export for %s truncated to %s rows", table, row_cap
                )
