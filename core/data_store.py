...
class EnhancedMusicDataStore:
    ...
    def get_tracks_with_artists_bulk(
        ...
        # Build a deterministic cache key across processes/runs.
        normalized_pairs = sorted((str(track), str(artist)) for track, artist in track_artist_pairs)
        key_material = json.dumps(normalized_pairs, separators=(",", ":"), ensure_ascii=True)
        cache_key = f"tracks_bulk:{hashlib.sha256(key_material.encode('utf-8')).hexdigest()}"
