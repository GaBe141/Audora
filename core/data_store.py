...
class EnhancedMusicDataStore:
    ...
    _ALLOWED_EXPORT_TABLE_FILTERED_QUERIES = {
        "trends": (
            "SELECT * FROM trends "
    ...
    def export_to_csv(self, table: str, filepath: str, days: int | None = None) -> str:
        ...
        """
        if table not in self._ALLOWED_EXPORT_TABLE_QUERIES:
            valid_tables = set(self._ALLOWED_EXPORT_TABLE_QUERIES)
            raise ValueError(f"Invalid table name: {table}. Must be one of {valid_tables}")

        with self.get_connection() as conn:
            if days:
                query = self._ALLOWED_EXPORT_TABLE_FILTERED_QUERIES[table]
                df = pd.read_sql_query(query, conn, params=[f"-{days} days"])
            else:
                query = self._ALLOWED_EXPORT_TABLE_QUERIES[table]
    ...
    def get_data_quality_report(self) -> dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            table_stats = {}
            for table, query in self._ALLOWED_TABLE_COUNT_QUERIES.items():
