...
class _ValidatedWebhookTarget:
    """Webhook target with the exact hosts approved during SSRF validation."""

    url: str
    hostname: str
    port: int
    resolved_hosts: tuple[dict[str, Any], ...]


class _PinnedWebhookResolver(aiohttp.abc.AbstractResolver):
    """Restrict aiohttp to the DNS answers already approved by validation."""

    def __init__(self, hostname: str, port: int, resolved_hosts: tuple[dict[str, Any], ...]):
    ...
    async def resolve(
        ...
        if host != self.hostname or port != self.port:
            raise OSError("Webhook resolver refused unexpected hostname")
        if family == socket.AF_UNSPEC:
            return list(self.resolved_hosts)
        matching_hosts = [entry for entry in self.resolved_hosts if entry["family"] == family]
        return matching_hosts or list(self.resolved_hosts)
...
class EnhancedNotificationService:
    ...
    def save_config(self, path: str = "config/notification_config.json") -> None:
        ...
        # Only persist non-secret preferences. Live credentials are loaded from
        # environment variables so repo-relative config files cannot leak them.
        to_save: dict[str, Any] = {
            "default_channels": self.config.get("default_channels", ["console"]),
            "rate_limit_per_hour": self.config.get("rate_limit_per_hour", 50),
            "email": {
                "port": self.config.get("email", {}).get("port", 587),
                "use_tls": self.config.get("email", {}).get("use_tls", True),
                "from_address": self.config.get("email", {}).get(
                    "from_address", "music-discovery@example.com"
                ),
            },
            "slack": {
                "channel": self.config.get("slack", {}).get("channel", "#music-trends"),
                "username": self.config.get("slack", {}).get(
                    "username", "Music Discovery Bot"
                ),
                "icon_emoji": self.config.get("slack", {}).get("icon_emoji", ":musical_note:"),
            },
            "discord": {
                "username": self.config.get("discord", {}).get("username", "Music Discovery"),
    ...
    def _validate_webhook_url(self, url: str, *, allow_private: bool = False) -> str:
        ...
        return self._validate_webhook_target(url, allow_private=allow_private).url

    def _validate_webhook_target(
        self, url: str, *, allow_private: bool = False
    ) -> _ValidatedWebhookTarget:
        """Validate outbound webhook URL and retain approved DNS answers."""
    ...
    def _create_validated_webhook_session(
        self, target: _ValidatedWebhookTarget
    ) -> aiohttp.ClientSession:
        """Create a client session pinned to the approved webhook DNS results."""
        resolver = _PinnedWebhookResolver(target.hostname, target.port, target.resolved_hosts)
...
""",
    ...
    async def _send_slack(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            target = self._validate_webhook_target(webhook_url, allow_private=False)
            ...
            async with (
                self._create_validated_webhook_session(target) as session,
                session.post(target.url, json=slack_message, allow_redirects=False) as response,
            ):
    ...
    async def _send_discord(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            async with (
                self._create_validated_webhook_session(target) as session,
                session.post(target.url, json=discord_message, allow_redirects=False) as response,
            ):
    ...
    async def _send_webhook(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            target = self._validate_webhook_target(
