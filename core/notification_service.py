...
class EnhancedNotificationService:
    ...
    def _sanitize_header_value(value: str) -> str:
        """Strip CR/LF so attacker-controlled values cannot inject email headers."""
        return value.replace("\r", "").replace("\n", "")

    def _allowed_attachment_roots(self) -> list[Path]:
        """Directories that email attachments may be read from."""
        cwd = Path.cwd().resolve()
        return [cwd, (cwd / "data").resolve(), (cwd / "exports").resolve()]

    def _is_allowed_attachment(self, attachment_path: Path) -> bool:
        """Return True when the path is a file under an allowed attachment root."""
        try:
...
""",
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            msg["To"] = self._sanitize_header_value(
                ", ".join(str(recipient) for recipient in email_config["recipients"])
            )
            ...
            if message.attachments:
                for attachment_path in message.attachments:
                    ...
                    if not self._is_allowed_attachment(path):
                        self.logger.warning(
                            "Skipped email attachment outside allowed directories: %s",
                            attachment_path,
                        )
                        continue
                    ...
                    with resolved.open("rb") as f:
                        ...
                        safe_name = self._sanitize_header_value(resolved.name)
