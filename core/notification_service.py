...
""",
    ...
    def _generate_message_key(self, message: NotificationMessage) -> str:
        ...
        # Use a deterministic cryptographic hash to avoid process-randomized
        # hash() behavior and reduce collision risk.
        digest_input = f"{message.title}:{message.content[:100]}:{message.priority.value}".encode(
            "utf-8"
        )
