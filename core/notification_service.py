...
class EnhancedNotificationService:
    ...
    def _sanitize_header_value(value: str) -> str:
        """Strip CR/LF so attacker-controlled fields cannot inject SMTP headers."""
        return "".join(ch for ch in value if ch not in "\r\n")

    @staticmethod
    def _allowed_attachment_roots() -> list[Path]:
        return [
            Path("data/exports").resolve(),
            Path("exports").resolve(),
        ]

    def _is_allowed_attachment(self, attachment_path: Path) -> bool:
        """Confine email attachments to export directories."""
        try:
            resolved = attachment_path.expanduser().resolve()
        except OSError:
            return False
        return any(
...
""",
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            recipients = [
                self._sanitize_header_value(str(recipient).strip())
            ...
            # Escape untrusted content so HTML clients cannot execute injected markup
            html_content = html.escape(text_content).replace("\n", "<br>")
            msg.attach(MIMEText(f"<html><body><pre>{html_content}</pre></body></html>", "html"))

            # Add attachments only from allowlisted export directories
            if message.attachments:
                for attachment_path in message.attachments:
                    path = Path(attachment_path)
                    if not path.exists() or not self._is_allowed_attachment(path):
                        self.logger.warning("Rejected email attachment outside export dirs: %s", path)
                        continue
                    with path.open("rb") as f:
                        attachment = MIMEBase("application", "octet-stream")
                        attachment.set_payload(f.read())
                        encoders.encode_base64(attachment)
                        safe_name = self._sanitize_header_value(path.name)
        ...
        try:
            ...
            async with (
                ...
                if 300 <= response.status < 400:
                    return {"success": False, "error": "Webhook redirect refused"}
