...
class EnhancedNotificationService:
    ...
    def _is_restricted_ip(self, ip: str) -> bool:
        """Return True when the IP is not a globally routable unicast address."""
        try:
            parsed = ipaddress.ip_address(ip)
            if parsed.version == 6 and parsed.ipv4_mapped is not None:
                parsed = parsed.ipv4_mapped
            # `is_global` also rejects CGNAT (100.64.0.0/10), docs, and other
            # non-public ranges that `is_private` does not cover.
    ...
    def _reject_redirect(self, status: int) -> None:
        """Fail closed when a webhook attempts to redirect after URL validation."""
        if 300 <= status < 400:
            raise ValueError("Webhook redirected; following redirects is disabled")

    def _resolve_safe_attachment(self, attachment_path: str) -> Path | None:
        """Allow email attachments only when they resolve inside the project root."""
        ...
        try:
            ...
            self.logger.warning(
                "Rejected attachment outside the project directory: %s", attachment_path
            )
            return None
...
""",
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            # Add HTML version if available (escape to prevent HTML injection)
    ...
    async def _send_slack(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            async with (
                ...
                self._reject_redirect(response.status)
