...
""",
    ...
    def _generate_message_key(self, message: NotificationMessage) -> str:
        ...
        # Use deterministic cryptographic hashing so keys are stable across processes.
        key_payload = f"{message.title}:{message.content[:100]}:{message.priority.value}"
        content_hash = hashlib.sha256(key_payload.encode("utf-8")).hexdigest()
        return content_hash
