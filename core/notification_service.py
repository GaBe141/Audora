...
class EnhancedNotificationService:
    ...
    def _is_allowed_attachment_path(self, path: str) -> bool:
        """Confine email attachments to the working tree export directories."""
...
""",
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            if not email_config.get("use_tls", True) and (
                email_config.get("username") or email_config.get("password")
            ):
                return {
                    "success": False,
            ...
            if message.attachments:
                for attachment_path in message.attachments:
                    if not self._is_allowed_attachment_path(attachment_path):
                        self.logger.warning(
                            "Skipped attachment outside allowed directories: %s",
                            attachment_path,
                        )
                        continue
                    if Path(attachment_path).exists():
                        with Path(attachment_path).open("rb") as f:
                            ...
                            safe_name = self._sanitize_header(Path(attachment_path).name)
