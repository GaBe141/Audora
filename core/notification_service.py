...
class EnhancedNotificationService:
    ...
    def _get_request_timeout(self, configured_timeout: Any = 30) -> aiohttp.ClientTimeout:
        """Build a bounded timeout for outbound notification requests."""
        try:
        ...
        try:
            ...
            async with (
                ...
                session.post(
                    webhook_url,
                    json=slack_message,
                    timeout=self._get_request_timeout(slack_config.get("timeout", 30)),
        ...
        try:
            ...
            async with (
                ...
                session.post(
                    ...
                    timeout=self._get_request_timeout(discord_config.get("timeout", 30)),
                    allow_redirects=False,
                ) as response,
            ):
        ...
        try:
            ...
            async with (
                ...
                session.post(
                    ...
                    json=payload,
                    headers=headers,
                    timeout=self._get_request_timeout(timeout),
                    allow_redirects=False,
                ) as response,
            ):
