...
class EnhancedNotificationService:
    ...
    def _normalize_ip(self, ip: str) -> ipaddress.IPv4Address | ipaddress.IPv6Address:
        """Unwrap IPv4-mapped IPv6 addresses so private ranges cannot be hidden."""
    ...
    def _is_restricted_ip(self, ip: str) -> bool:
        """Return True when the IP is not globally routable (private, loopback, CGNAT, etc.)."""
        try:
    ...
    def _validate_webhook_url(self, url: str, *, allow_private: bool = False) -> str:
        ...
        if hostname_l == "localhost" or hostname_l.endswith(".localhost"):
    ...
    def _is_safe_attachment_path(self, attachment_path: str) -> bool:
        """Allow attachments only when they resolve inside the project root."""
...
""",
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            # Add HTML version if available (escape untrusted template output)
            ...
            if message.attachments:
                for attachment_path in message.attachments:
                    if not self._is_safe_attachment_path(attachment_path):
                        self.logger.warning(
                            "Skipped attachment outside project root: %s", attachment_path
                        )
                        continue
        ...
        try:
            ...
            async with (
                ...
                if 300 <= response.status < 400:
                    self.logger.error("Slack notification redirect rejected: %s", response.status)
                    return {
                        "success": False,
    ...
    async def _send_webhook(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            async with (
                ...
                if 300 <= response.status < 400:
                    self.logger.error("Webhook notification redirect rejected: %s", response.status)
                    return {
                        "success": False,
                        "error": f"Redirect rejected: HTTP {response.status}",
                    }
