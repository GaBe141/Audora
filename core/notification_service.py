...
class EnhancedNotificationService:
    ...
    def save_config(self, path: str = "config/notification_config.json") -> None:
        ...
        to_save = {k: deepcopy(self.config[k]) for k in saveable_keys if k in self.config}
        if "email" in to_save:
            to_save["email"].pop("password", None)
        if "webhook" in to_save and isinstance(to_save["webhook"].get("headers"), dict):
            to_save["webhook"]["headers"].pop("Authorization", None)
        if "sms" in to_save:
