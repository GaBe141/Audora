...
class _WebhookTarget:
    """Validated webhook URL plus the IPs that were security-checked."""
...
class _PinnedWebhookResolver:
    """aiohttp resolver that pins a request to pre-validated DNS answers."""
    ...
    async def resolve(
        ...
        **_kwargs: Any,
        ...
        if host != self.hostname:
            raise OSError("Refusing to resolve unvalidated webhook hostname")
...
class EnhancedNotificationService:
    ...
    def _validate_webhook_target(self, url: str, *, allow_private: bool = False) -> _WebhookTarget:
        """Validate outbound webhook URL and capture DNS answers to reduce SSRF risk."""
        ...
        return _WebhookTarget(url=url, hostname=hostname, resolved_ips=tuple(sorted(resolved_ips)))
    ...
    async def _post_validated_webhook_json(
        self,
        ...
        *,
        ...
        """Post JSON to a pre-validated webhook without re-resolving DNS."""
        connector = aiohttp.TCPConnector(
            resolver=_PinnedWebhookResolver(target.hostname, target.resolved_ips)
        )
        async with (
            aiohttp.ClientSession(connector=connector) as session,
            session.post(target.url, json=payload, headers=headers, timeout=timeout) as response,
        ):
...
""",
    ...
    async def _send_slack(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            status, response_text = await self._post_validated_webhook_json(target, slack_message)
            if status == 200:
    ...
    async def _send_discord(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            status, response_text = await self._post_validated_webhook_json(target, discord_message)
    ...
    async def _send_webhook(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            status, response_text = await self._post_validated_webhook_json(
                target,
                payload,
                headers=headers,
