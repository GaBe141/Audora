...
""",
    ...
    def _safe_attachment_path(self, attachment_path: str) -> Path | None:
        """Return a resolved attachment path only if it stays inside data/."""
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            html_content = html.escape(str(text_content)).replace("\n", "<br>")
            msg.attach(MIMEText(f"<html><body><pre>{html_content}</pre></body></html>", "html"))

            # Add attachments from the data directory only
