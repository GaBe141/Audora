...
""",
    ...
    def _safe_attachment_path(self, attachment_path: str) -> Path | None:
        """Return a resolved attachment path if it stays under data/exports."""
        allowed_root = (Path.cwd() / "data" / "exports").resolve()
        resolved = Path(attachment_path).expanduser().resolve()
        try:
            resolved.relative_to(allowed_root)
        except ValueError:
            self.logger.warning("Rejected attachment outside data/exports: %s", attachment_path)
            return None
    ...
    def _reject_redirect_status(self, status: int) -> dict[str, Any] | None:
        """Reject HTTP redirect responses that could enable SSRF chaining."""
        if 300 <= status < 400:
            return {
                "success": False,
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            recipients = [
                self._sanitize_header_value(str(recipient))
            ...
            escaped_html = html.escape(text_content)
            ...
            if message.attachments:
                for attachment_path in message.attachments:
                    ...
                    with safe_path.open("rb") as f:
                        ...
                        filename = self._sanitize_header_value(safe_path.name)
