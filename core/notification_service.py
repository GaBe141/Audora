...
""",
    ...
    def _generate_message_key(self, message: NotificationMessage) -> str:
        ...
        # Stable digest avoids Python hash randomization and collision surprises.
        dedup_source = f"{message.title}\n{message.content[:100]}\n{message.priority.value}"
        content_hash = hashlib.sha256(dedup_source.encode("utf-8")).hexdigest()
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            # Send email with verified TLS by default.
