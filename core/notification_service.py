...
class EnhancedNotificationService:
    ...
    def _resolve_attachment_path(self, attachment_path: str) -> Path | None:
        """Resolve attachment paths, rejecting files outside allowlisted directories."""
        try:
            target = Path(attachment_path).expanduser().resolve()
        except OSError:
            self.logger.warning("Rejected unreadable attachment path: %s", attachment_path)
            return None
        ...
        if not target.is_file() or not any(
            self._is_relative_to(target, root) for root in allowed_roots
        ):
            self.logger.warning("Rejected attachment outside allowed directories: %s", target)
            return None
        return target
