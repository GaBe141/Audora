...
class EnhancedNotificationService:
    ...
    def _is_restricted_ip(self, ip: str) -> bool:
        ...
        try:
            ...
            # reserved, unspecified, and CGNAT (100.64.0.0/10) ranges.
    ...
    def _resolve_attachment_path(self, attachment_path: str) -> Path | None:
        """Resolve an email attachment only if it stays inside the project root."""
...
""",
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            # Escape untrusted content before wrapping it in HTML.
            html_content = html.escape(text_content).replace("\n", "<br>")
            msg.attach(MIMEText(f"<html><body><pre>{html_content}</pre></body></html>", "html"))

            # Add attachments confined to the project tree.
            if message.attachments:
                for attachment_path in message.attachments:
                    resolved = self._resolve_attachment_path(attachment_path)
                    if resolved is None:
                        continue
            ...
            if username and password and not use_tls:
                return {
                    "success": False,
                    "error": "Refusing to authenticate over plaintext SMTP; enable TLS",
                }
