...
class EnhancedNotificationService:
    ...
    def _is_allowed_attachment(self, attachment_path: Path) -> bool:
        """Confine email attachments to data/ and exports/ directories."""
        try:
...
""",
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            # Add HTML version with escaped user-controlled content
            ...
            if message.attachments:
                for attachment_path in message.attachments:
                    ...
                    if not path.exists() or not self._is_allowed_attachment(path):
                        self.logger.warning(
                            "Skipping attachment outside allowed directories: %s",
                            attachment_path,
                        )
                        continue
                    with path.open("rb") as f:
                        ...
                        attachment.add_header(
                            "Content-Disposition",
                            f"attachment; filename= {self._sanitize_header(path.name)}",
                        )
