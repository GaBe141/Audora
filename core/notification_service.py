...
class EnhancedNotificationService:
    ...
    def _is_allowed_attachment(self, attachment_path: Path) -> bool:
        """Allow attachments only from data/, exports/, or config/."""
        try:
...
""",
    ...
    async def _send_email(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            # Add HTML version with escaped content to prevent XSS
            html_content = html.escape(text_content).replace("\n", "<br>")
            msg.attach(MIMEText(f"<html><body><pre>{html_content}</pre></body></html>", "html"))

            # Add attachments confined to known project directories
            if message.attachments:
                for attachment_path in message.attachments:
                    path = Path(attachment_path)
                    if not path.exists():
                        self.logger.warning(f"Skipping missing attachment: {attachment_path}")
                        continue
                    if not self._is_allowed_attachment(path):
                        self.logger.warning(
                            f"Skipping attachment outside allowed directories: {attachment_path}"
                        )
                        continue
        ...
        try:
            ...
            async with (
                ...
                if 300 <= response.status < 400:
                    self.logger.error(
                        f"Slack notification rejected redirect: {response.status}"
                    )
    ...
    async def _send_discord(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            async with (
                ...
                if 300 <= response.status < 400:
                    self.logger.error(
                        f"Discord notification rejected redirect: {response.status}"
                    )
                    return {"success": False, "error": f"Redirect not allowed: HTTP {response.status}"}
    ...
    async def _send_webhook(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            async with (
                ...
                if 300 <= response.status < 400:
                    self.logger.error(
                        f"Webhook notification rejected redirect: {response.status}"
                    )
                    return {"success": False, "error": f"Redirect not allowed: HTTP {response.status}"}
