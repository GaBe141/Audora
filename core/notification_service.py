...
class EnhancedNotificationService:
    ...
    async def _post_json_no_redirects(
        ...
        *,
        ...
        """Send JSON payload with redirects disabled to prevent SSRF bypass."""
        try:
    ...
    async def _send_slack(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            status_code, response_text = await self._post_json_no_redirects(
                webhook_url,
                slack_message,
                timeout_seconds=slack_config.get("timeout", 30),
            )
            if status_code == 200:
    ...
    async def _send_discord(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            status_code, response_text = await self._post_json_no_redirects(
                webhook_url,
                discord_message,
                timeout_seconds=discord_config.get("timeout", 30),
            )
