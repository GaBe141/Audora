...
class EnhancedNotificationService:
    ...
    def _reject_redirect_status(self, status: int) -> None:
        """Reject HTTP redirect responses that could be used for SSRF bypass."""
        if 300 <= status < 400:
            raise ValueError("Webhook redirect responses are not allowed")

    def _safe_attachment_path(self, attachment_path: str) -> Path | None:
        """Return a project-rooted attachment path, or None if it is unsafe."""
        project_root = Path(__file__).resolve().parent.parent
        try:
            candidate = Path(attachment_path).expanduser().resolve(strict=False)
