...
class EnhancedNotificationService:
    ...
    async def _post_webhook_json(
        ...
        *,
        ...
        """Send webhook payload with hardened HTTP defaults.

        - Disable redirects to prevent SSRF bypass via redirect chains.
        - Enforce a conservative timeout budget.
        """
        timeout = aiohttp.ClientTimeout(
            total=timeout_seconds,
            connect=min(timeout_seconds, 5),
            sock_connect=min(timeout_seconds, 5),
            sock_read=max(1, timeout_seconds),
        )
        async with (
...
""",
    ...
    async def _send_slack(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            status_code, response_text = await self._post_webhook_json(
                webhook_url, slack_message, timeout_seconds=10
            )
            if status_code == 200:
    ...
    async def _send_discord(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            status_code, response_text = await self._post_webhook_json(
                webhook_url, discord_message, timeout_seconds=10
            )
    ...
    async def _send_webhook(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            timeout = max(1, min(int(webhook_config.get("timeout", 30)), 30))
