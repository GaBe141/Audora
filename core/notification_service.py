...
class EnhancedNotificationService:
    ...
    def _validate_webhook_url(self, url: str, *, allow_private: bool = False) -> str:
        ...
        if parsed.username or parsed.password:
            raise ValueError("Webhook URLs with embedded credentials are not allowed")
    ...
    def _safe_attachment_path(self, attachment_path: str) -> Path | None:
        """Return a resolved path only when it is a file under an allowed directory."""
        try:
    ...
    def _reject_redirect_response(self, response: aiohttp.ClientResponse) -> None:
        """Reject HTTP redirects so validated webhook hosts cannot bounce to private networks."""
        if 300 <= response.status < 400:
            raise ValueError("Webhook redirect responses are not allowed")
    ...
    async def _send_slack(self, message: NotificationMessage) -> dict[str, Any]:
        ...
        try:
            ...
            async with (
                ...
                self._reject_redirect_response(response)
