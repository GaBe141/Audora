...
_CACHE_ENVELOPE_ALG = "HMAC-SHA256"
...
def _coerce_jsonable(value: Any) -> Any:
    """Convert common numeric/date objects into JSON-serializable values."""
...
def _encode_cache_value(value: Any) -> tuple[str, Any]:
    """Encode a cache value into a JSON-safe kind/payload pair."""
    if isinstance(value, (bytes, bytearray, memoryview)):
        encoded = base64.b64encode(bytes(value)).decode("ascii")
        return "bytes", encoded
    if isinstance(value, pd.DataFrame):
        frame_payload = json.loads(value.to_json(orient="split", date_format="iso"))
        return "pandas_dataframe", frame_payload
    return "json", _coerce_jsonable(value)


def _decode_cache_value(kind: str, payload: Any) -> Any:
    """Decode a kind/payload pair produced by `_encode_cache_value`."""
    if kind == "json":
        return payload
    if kind == "bytes":
        if not isinstance(payload, str):
            raise ValueError("Bytes cache payload must be a base64 string")
        return base64.b64decode(payload.encode("ascii"), validate=True)
    if kind == "pandas_dataframe":
        if not isinstance(payload, dict):
            raise ValueError("DataFrame cache payload must be an object")
        return pd.read_json(StringIO(json.dumps(payload)), orient="split")
    raise ValueError(f"Unsupported cache payload kind: {kind}")


def _canonical_cache_body(kind: str, payload: Any) -> bytes:
    """Build the canonical bytes that are HMAC-signed for a cache envelope."""
    return json.dumps(
        {"v": CACHE_ENVELOPE_VERSION, "kind": kind, "payload": payload},
        separators=(",", ":"),
        sort_keys=True,
    ).encode("utf-8")


def serialize_cache_value(signing_key: bytes, value: Any) -> bytes:
    """Serialize a cache value into an HMAC-signed JSON envelope."""
    kind, payload = _encode_cache_value(value)
    signature = hmac.new(
        signing_key, _canonical_cache_body(kind, payload), hashlib.sha256
    ).hexdigest()
    envelope = {
        "v": CACHE_ENVELOPE_VERSION,
        "alg": _CACHE_ENVELOPE_ALG,
        "sig": signature,
        "kind": kind,
        "payload": payload,
    }
...
def deserialize_cache_value(signing_key: bytes, raw: bytes) -> Any:
    """Deserialize an HMAC-signed JSON cache envelope.

    Raises:
        ValueError: If the payload is malformed, unsigned, tampered, or legacy.
    """
    if not isinstance(raw, (bytes, bytearray)):
        raise ValueError("Cache payload must be bytes")
    ...
    if (
        ...
        or envelope.get("alg") != _CACHE_ENVELOPE_ALG
        or envelope.get("kind") not in _SUPPORTED_CACHE_KINDS
        or "sig" not in envelope
        or "payload" not in envelope
    ):
        raise ValueError("Cache payload envelope is invalid or unsupported")

    signature = envelope.get("sig")
    if not isinstance(signature, str):
        raise ValueError("Cache payload signature is invalid")

    expected_sig = hmac.new(
        signing_key,
        _canonical_cache_body(envelope["kind"], envelope["payload"]),
        hashlib.sha256,
    ).hexdigest()
    if not hmac.compare_digest(signature, expected_sig):
        raise ValueError("Cache payload signature mismatch")

    return _decode_cache_value(envelope["kind"], envelope["payload"])
...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        return serialize_cache_value(self._signing_key, value)

    def _deserialize(self, value: bytes) -> Any:
        """Deserialize cache value only after signature verification."""
        return deserialize_cache_value(self._signing_key, value)
    ...
    def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Rejected cache entry for %s: %s", key, e)
                self.delete(key)
                return None
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        try:
            ...
            logger.error(f"Redis set skipped unsupported value for key {key}: {e}")
        except Exception as e:
