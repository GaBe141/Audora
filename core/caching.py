...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value without unsafe code-executing formats."""
    ...
    def _to_safe_envelope(self, value: Any) -> dict[str, Any]:
        """Build a versioned envelope for supported cache payloads."""
        if self._is_json_safe(value):
            return {"v": 2, "type": "json", "payload": value}
    ...
    def _from_safe_envelope(self, envelope: dict[str, Any]) -> Any | None:
        """Read a supported cache payload from a versioned envelope."""
        payload_type = envelope.get("type")
        payload = envelope.get("payload")

        if payload_type == "json" and self._is_json_safe(payload):
            return payload

        if payload_type == "bytes" and isinstance(payload, str):
            return base64.b64decode(payload.encode("ascii"), validate=True)

        if payload_type == "pandas_dataframe" and isinstance(payload, str):
            import pandas as pd
    ...
    def _is_json_safe(self, value: Any) -> bool:
        """Return True for values JSON can represent without custom object hooks."""
        if value is None or isinstance(value, (str, int, float, bool)):
            return True
        if isinstance(value, list):
            return all(self._is_json_safe(item) for item in value)
        if isinstance(value, dict):
            return all(isinstance(key, str) and self._is_json_safe(item) for key, item in value.items())
        return False
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (JSON-safe, bytes, or pandas DataFrame for Redis)
