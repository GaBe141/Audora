...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for explicit JSON envelopes
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize supported cache values without unsafe object deserialization."""
        envelope: dict[str, Any] = {"v": 2}
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize only strict JSON cache envelopes."""
        try:
            ...
            logger.error(f"Failed to deserialize cache entry safely: {e}")
            return None

    def _delete_invalid_entry(self, key: str) -> None:
        """Remove invalid cache values that cannot be safely deserialized."""
        try:
