...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        Args:
            ...
            key_namespace: Prefix used when clearing keys (never FLUSHDB)
        """
    ...
    def _canonical_payload(self, payload_type: str, data: Any) -> bytes:
        """Build a stable byte representation for HMAC signing."""
        return json.dumps(
            {"data": data, "type": payload_type},
            separators=(",", ":"),
            sort_keys=True,
        ).encode("utf-8")

    def _encode_value(self, value: Any) -> tuple[str, Any]:
        """Convert a cache value into a JSON-safe payload type and data."""
        if isinstance(value, pd.DataFrame):
            return "pandas_dataframe", json.loads(
                value.to_json(orient="split", date_format="iso")
            )
        if isinstance(value, (bytes, bytearray)):
            return "bytes", base64.b64encode(bytes(value)).decode("ascii")

        # Reject values that are not JSON-serializable. Never fall back to pickle.
        json.dumps(value)
        return "json", value

    def _decode_value(self, payload_type: str, data: Any) -> Any | None:
        """Convert a verified JSON payload back into a Python value."""
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        payload_type, payload = self._encode_value(value)
        signature = hmac.new(
            self._signing_key,
            self._canonical_payload(payload_type, payload),
            hashlib.sha256,
        ).hexdigest()
        envelope = {
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            if (
                ...
                or payload_type not in _SUPPORTED_PAYLOAD_TYPES
            ...
            return self._decode_value(payload_type, payload)
    ...
    def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            if deserialized is None:
                # Drop legacy pickle / tampered / malformed entries.
                self.delete(key)
            return deserialized
        except Exception as e:
    ...
    def clear(self) -> None:
        """Clear cached values in this namespace only. Never FLUSHDB."""
        try:
            pattern = f"{self._key_namespace}:*"
            cursor = 0
            deleted = 0
            while True:
...
class CacheManager:
    ...
    def __init__(
        ...
        if backend:
            ...
            if REDIS_AVAILABLE:
                try:
                    self._backend = RedisCacheBackend(key_namespace=key_prefix)
