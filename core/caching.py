...
class RedisCacheBackend(CacheBackend):
    ...
    _SIGNING_ENV_VAR = "AUDORA_CACHE_SIGNING_KEY"
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for signed serialized payloads
        )
    ...
    def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            logger.warning(f"Rejected unsigned or tampered cache entry for key {key}: {e}")
            return None
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        """Set value in cache with optional TTL."""
        try:
            serialized = self._serialize_for_cache(value)
            if ttl:
    ...
    @classmethod
    def _load_signing_key(cls) -> bytes:
        """Load signing key used to protect cache payload integrity."""
        configured_key = os.getenv(cls._SIGNING_ENV_VAR, "").strip()
        if configured_key:
            return hashlib.sha256(configured_key.encode("utf-8")).digest()

        logger.warning(
            "AUDORA_CACHE_SIGNING_KEY is not set; using process-local ephemeral key "
            "for Redis cache signing. Set this variable to enable stable cross-process cache reads."
        )
        return os.urandom(32)

    def _serialize_for_cache(self, value: Any) -> bytes:
        """Serialize and sign cache value to prevent payload tampering."""
        payload = pickle.dumps(value, protocol=pickle.HIGHEST_PROTOCOL)
        signature = hmac.new(self._signing_key, payload, hashlib.sha256).hexdigest().encode("ascii")
        return signature + b":" + payload

    def _deserialize_from_cache(self, serialized_value: bytes) -> Any:
        """Verify signature and deserialize cache value."""
        signature, separator, payload = serialized_value.partition(b":")
        if not separator:
