...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value using a safe, typed JSON envelope."""
        try:
            import pandas as pd
        ...
        if isinstance(value, bytes | bytearray):
            ...
            # Strict JSON serialization keeps Redis cache reads from executing code.
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize a cache value without executing code from Redis."""
        try:
