...
_SUPPORTED_JSON_TYPES = (dict, list, str, int, float, bool)
...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        Args:
            ...
            key_prefix: Prefix used when clearing cache entries
        """
    ...
    def _mac(self, kind: str, payload: bytes) -> str:
        """HMAC-SHA256 over payload type + body to prevent type confusion."""
        return hmac.new(
            self._signing_key,
            kind.encode("utf-8") + b"\0" + payload,
            hashlib.sha256,
        ).hexdigest()

    def _encode_value(self, value: Any) -> tuple[str, bytes]:
        """Encode a cache value as a safe JSON/bytes/DataFrame payload."""
        if isinstance(value, _SUPPORTED_JSON_TYPES) or value is None:
            return "json", json.dumps(value, separators=(",", ":")).encode("utf-8")
        if isinstance(value, bytes):
            return "bytes", value
        if isinstance(value, pd.DataFrame):
            return "pandas_dataframe", value.to_json(orient="split").encode("utf-8")
        raise TypeError(f"Unsupported cache value type: {type(value)!r}")

    def _decode_value(self, kind: str, payload: bytes) -> Any:
        ...
        raise ValueError(f"Unsupported cache payload type: {kind}")

    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value as an HMAC-signed JSON envelope."""
        kind, payload = self._encode_value(value)
        envelope = {
            "v": _CACHE_ENVELOPE_VERSION,
            "alg": "HMAC-SHA256",
            "type": kind,
            "sig": self._mac(kind, payload),
    ...
    def clear(self) -> None:
        """Clear cache keys scoped to this backend prefix. Never FLUSHDB."""
        try:
