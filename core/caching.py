...
def _decode_cache_payload(kind: str, data: Any) -> Any:
    """Restore a cache value from a typed JSON payload."""
    if kind == "bytes":
...
def serialize_cache_value(value: Any, signing_key: bytes) -> bytes:
    ...
    encoded = _encode_cache_payload(value)
    body_obj = {
...
def deserialize_cache_value(raw: bytes, signing_key: bytes) -> Any | None:
    """Deserialize a signed JSON cache envelope. Rejects legacy pickle payloads."""
    try:
    ...
    # Legacy HMAC-signed pickle envelopes used v=1 and a "payload" field.
    if envelope.get("v") != CACHE_ENVELOPE_VERSION or "payload" in envelope:
        logger.warning("Rejected legacy or unsupported cache serialization format")
        return None
    ...
    expected_sig = hmac.new(signing_key, body_bytes, hashlib.sha256).hexdigest()
    ...
    try:
        ...
        if (
            ...
            or body.get("v") != CACHE_ENVELOPE_VERSION
            or "kind" not in body
            or "data" not in body
        ):
            logger.warning("Rejected cache entry with invalid signed body")
            return None
        return _decode_cache_payload(str(body["kind"]), body["data"])
    except Exception as exc:
        logger.error(f"Failed to deserialize cache entry: {exc}")
        return None
...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        Args:
            ...
            key_prefix: Prefix used to scope bulk deletes
        """
    ...
    def clear(self) -> None:
        ...
        Uses SCAN + DELETE instead of FLUSHDB so other databases/keys are preserved.
        """
        try:
