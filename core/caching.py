...
_PICKLE_ENABLED_VALUES = {"1", "true", "yes", "on"}


class _RestrictedUnpickler(pickle.Unpickler):
    """Restricted unpickler that only permits simple built-in containers."""
...
def _restricted_pickle_loads(payload: bytes) -> Any:
    """Safely load limited pickle payloads."""
...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        JSON is used by default to avoid pickle RCE risk. Legacy pickle support is
        optional and only enabled with AUDORA_CACHE_ALLOW_PICKLE=true.
        """
        payload_format = "json"
        try:
            payload = json.dumps(value, separators=(",", ":"), sort_keys=True).encode("utf-8")
        except (TypeError, ValueError):
            if not self._allow_pickle_payloads():
                raise ValueError(
                    "Cache value is not JSON serializable. "
                    "Set AUDORA_CACHE_ALLOW_PICKLE=true only if you trust your Redis boundary."
                )
            payload_format = "pickle"
            payload = pickle.dumps(value, protocol=pickle.HIGHEST_PROTOCOL)

        signed_message = payload_format.encode("ascii") + b":" + payload
        signature = hmac.new(self._signing_key, signed_message, hashlib.sha256).hexdigest()
        envelope = {
            "v": 2,
            "alg": "HMAC-SHA256",
    ...
    def _allow_pickle_payloads(self) -> bool:
        """Whether unsafe pickle payload support is explicitly enabled."""
        return os.getenv("AUDORA_CACHE_ALLOW_PICKLE", "").strip().lower() in _PICKLE_ENABLED_VALUES

    def _deserialize_v1_envelope(self, envelope: dict[str, Any]) -> Any | None:
        """Deserialize legacy v1 signed pickle envelope."""
        if not self._allow_pickle_payloads():
            logger.warning(
                "Rejected legacy pickle cache entry. Set AUDORA_CACHE_ALLOW_PICKLE=true to enable."
            )
            return None
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            if envelope.get("v") == 1 and envelope.get("alg") == "HMAC-SHA256":
                return self._deserialize_v1_envelope(envelope)

            if (
                envelope.get("v") != 2
                or envelope.get("alg") != "HMAC-SHA256"
                or "format" not in envelope
            ...
            payload_format = envelope["format"]
            if payload_format not in {"json", "pickle"}:
            ...
            payload = base64.b64decode(payload_b64.encode("ascii"), validate=True)
            signed_message = payload_format.encode("ascii") + b":" + payload
            expected_sig = hmac.new(self._signing_key, signed_message, hashlib.sha256).hexdigest()
            ...
            if not self._allow_pickle_payloads():
                logger.warning(
                    "Rejected pickle cache entry. Set AUDORA_CACHE_ALLOW_PICKLE=true to enable."
                )
                return None
            return _restricted_pickle_loads(payload)
