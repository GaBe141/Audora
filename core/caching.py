...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(value: Any) -> bytes | None:
        ...
        Returns None for non-JSON-serializable values to avoid unsafe fallbacks.
        """
        try:
            return json.dumps(value, ensure_ascii=True, separators=(",", ":")).encode("utf-8")
        except (TypeError, ValueError) as e:
            logger.warning(f"Skipping cache set for non-JSON-serializable value: {e}")
            return None

    @staticmethod
    def _deserialize(value: bytes, key: str) -> Any | None:
        """Deserialize cache JSON payload from bytes."""
        try:
            return json.loads(value.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as e:
            logger.warning(f"Discarding invalid cached payload for key {key}: {e}")
            return None
