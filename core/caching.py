...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value to a safe, typed JSON envelope.

        Redis content is not a trusted boundary, so avoid executable formats such as pickle.
        """
        if isinstance(value, bytes):
            ...
            serialized_payload = json.dumps(value, separators=(",", ":"))
            payload = json.loads(serialized_payload)
            if payload != value:
                raise TypeError("JSON serialization would change the cached value")
            envelope = {"v": 1, "type": "json", "payload": payload}
        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize a cache value from a safe, typed JSON envelope."""
        try:
            ...
            if payload_type == "pandas_dataframe":
                if not PANDAS_AVAILABLE:
                    logger.warning(
                        "Rejected pandas DataFrame cache entry because pandas is unavailable"
                    )
                    return None
                if not isinstance(payload, str):
    ...
    def _delete_invalid_entry(self, key: str) -> None:
        """Best-effort deletion for invalid Redis entries."""
        try:
            self._client.delete(key)
        except Exception as e:
            logger.error(f"Redis delete error for invalid key {key}: {e}")

    def _safe_serialize(self, value: Any) -> bytes | None:
        """Serialize a value or return None when the type is unsupported."""
        try:
            return self._serialize(value)
        except (TypeError, ValueError) as e:
            logger.warning("Unsupported cache value type %s: %s", type(value).__name__, e)
            return None
