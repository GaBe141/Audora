...
def _encode_cache_value(value: Any) -> dict[str, Any]:
    """Encode a cache value as a JSON-safe typed payload (no pickle)."""
    ...
    try:
        ...
        raise TypeError(f"Unsupported cache value type: {type(value).__name__}") from exc
    return {"kind": "json", "data": value}


def _decode_cache_value(encoded: dict[str, Any]) -> Any:
    """Decode a typed JSON cache payload."""
    ...
    if kind == "bytes":
        if not isinstance(data, str):
            raise UnsafeCachePayloadError("invalid bytes payload")
        return base64.b64decode(data.encode("ascii"), validate=True)
    if kind == "pandas_dataframe":
        if not isinstance(data, dict):
            raise UnsafeCachePayloadError("invalid dataframe payload")
        return pd.read_json(StringIO(json.dumps(data)), orient="split")
    raise UnsafeCachePayloadError(f"unsupported cache payload kind: {kind}")
...
class RedisCacheBackend(CacheBackend):
    ...
    def _deserialize(self, value: bytes) -> Any:
        """Deserialize cache value only after signature and type-envelope verification."""
        try:
            envelope = json.loads(value.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise UnsafeCachePayloadError("cache entry is not valid JSON") from exc
        ...
        if (
            ...
            raise UnsafeCachePayloadError("invalid serialization envelope")

        try:
            payload = base64.b64decode(envelope["payload"].encode("ascii"), validate=True)
        except (ValueError, UnicodeEncodeError) as exc:
            raise UnsafeCachePayloadError("invalid payload encoding") from exc

        expected_sig = hmac.new(self._signing_key, payload, hashlib.sha256).hexdigest()
        try:
            signature_valid = hmac.compare_digest(envelope["sig"], expected_sig)
        except ValueError as exc:
            raise UnsafeCachePayloadError("invalid signature") from exc
        if not signature_valid:
            raise UnsafeCachePayloadError("invalid signature")

        try:
            encoded = json.loads(payload.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise UnsafeCachePayloadError("signed payload is not valid JSON") from exc

        if not isinstance(encoded, dict):
            raise UnsafeCachePayloadError("signed payload must be an object")

        return _decode_cache_value(encoded)
    ...
    def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            except UnsafeCachePayloadError as e:
                logger.warning(f"Rejected unsafe Redis cache entry for key {key}: {e}")
