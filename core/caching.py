...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep bytes for explicit UTF-8 decoding
        )
    ...
    def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            # Treat malformed/legacy cache entries as misses and remove them.
            logger.warning(f"Redis cache entry for key {key} is not valid JSON; deleting entry")
            self.delete(key)
            return None
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Redis set skipped for key {key}: value is not JSON-serializable"
            )
        except Exception as e:
