...
class RedisCacheBackend(CacheBackend):
    ...
    def _get_signing_key(self) -> bytes:
        ...
        # Fallback to process-local random key keeps signed payload validation enabled.
    ...
    def _json_default(self, value: Any) -> Any:
        """Encode non-JSON-native values into safe tagged JSON structures."""
        ...
        if HAS_PANDAS and isinstance(value, pd.DataFrame):
            return {
                "__audora_cache_type__": "pandas_dataframe_split",
    ...
    def _json_object_hook(self, obj: dict[str, Any]) -> Any:
        """Decode tagged JSON structures back into Python objects."""
        type_marker = obj.get("__audora_cache_type__")
        if type_marker == "datetime":
            ...
            if isinstance(value, str):
                try:
                    ...
                    return obj
        elif type_marker == "pandas_dataframe_split":
            value = obj.get("value")
            if isinstance(value, str) and HAS_PANDAS:
                try:
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (must be JSON-serializable or a supported typed object)
