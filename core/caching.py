...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for explicit JSON bytes payloads
        )
    ...
    def _serialize_cache_value(value: Any) -> bytes:
        """Serialize cache values using safe JSON instead of pickle."""
        payload = {"value": value}
        return json.dumps(payload, default=str, separators=(",", ":")).encode("utf-8")

    @staticmethod
    def _deserialize_cache_value(serialized_value: bytes) -> Any | None:
        """Deserialize cache payload safely; return None on malformed data."""
        try:
            payload = json.loads(serialized_value.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError):
            logger.warning("Skipping malformed cache payload")
            return None

        if not isinstance(payload, dict) or "value" not in payload:
            logger.warning("Skipping cache payload with unexpected structure")
            return None
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (must be JSON-serializable or string-castable)
