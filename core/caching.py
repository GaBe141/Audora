...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        envelope = {"v": 2, "format": "json", "payload": self._encode_value(value)}
    ...
    def _encode_value(self, value: Any) -> Any:
        """Encode supported Python values using JSON-safe type markers."""
        if value is None or isinstance(value, (bool, int, float, str)):
            return value
        if isinstance(value, bytes):
            return {"__type__": "bytes", "data": base64.b64encode(value).decode("ascii")}
        if isinstance(value, tuple):
            return {"__type__": "tuple", "items": [self._encode_value(item) for item in value]}
        if isinstance(value, set):
            return {"__type__": "set", "items": [self._encode_value(item) for item in value]}
    ...
    def _decode_value(self, value: Any) -> Any:
        """Decode JSON-safe cache values without executing arbitrary code."""
        if value is None or isinstance(value, (bool, int, float, str)):
            return value
        if isinstance(value, list):
        ...
        if isinstance(value, dict):
            ...
            if marker == "pandas_series":
                ...
                return pd.read_json(io.StringIO(data), orient="split", typ="series")
            if marker is not None:
                raise ValueError(f"Unsupported cache payload marker: {marker}")
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (must be JSON-compatible or a supported safe type)
