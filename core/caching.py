...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        if isinstance(value, bytes):
            ...
            envelope = {"v": 2, "kind": "json", "payload": value}
        ...
        try:
            ...
            if kind == "bytes":
                ...
                if not isinstance(payload, str):
                    logger.warning("Rejected byte cache entry with non-string payload")
                    return None
            ...
            logger.warning("Rejected cache entry with unsupported serialization kind")
            return None
    ...
    def _delete_invalid_entry(self, key: str) -> None:
        """Remove invalid cache entries after rejecting their serialized payload."""
        try:
            self._client.delete(key)
        except Exception as e:
            logger.debug(f"Failed to delete invalid Redis cache entry {key}: {e}")
