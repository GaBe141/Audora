...
class RedisCacheBackend(CacheBackend):
    ...
    def _canonical_json_bytes(self, value: Any) -> bytes:
        """Serialize JSON value deterministically for signing."""
        return json.dumps(value, separators=(",", ":"), sort_keys=True).encode("utf-8")

    def _encode_payload(self, value: Any) -> dict[str, Any]:
        """Encode cache value using safe, allow-listed serialization."""
        try:
            ...
            # If pandas is unavailable, we simply fall through to JSON-compatible types.
            pass
    ...
    def _decode_payload(self, payload: dict[str, Any]) -> Any:
        """Decode previously encoded cache payload."""
        payload_type = payload.get("type")
        if payload_type == "json":
            return payload.get("value")

        if payload_type == "pandas.DataFrame":
            frame_json = payload.get("value")
        ...
        raise ValueError(f"Unsupported cached payload type: {payload_type}")
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            if not isinstance(payload, dict):
                logger.warning("Rejected cache entry with non-dict payload envelope")
                return None
