...
class RedisCacheBackend(CacheBackend):
    ...
    def _to_json_envelope(self, value: Any) -> dict[str, Any]:
        """Convert supported cache values to explicit JSON-safe envelopes."""
    ...
    def _from_json_envelope(self, envelope: dict[str, Any]) -> Any | None:
        """Restore values from explicit JSON-safe envelopes."""
        ...
        if value_type == "bytes":
            if not isinstance(value, str):
                raise ValueError("Bytes cache payload must be a string")
            return base64.b64decode(value.encode("ascii"), validate=True)
        if value_type == "pandas.DataFrame":
            if not isinstance(value, str):
                raise ValueError("DataFrame cache payload must be a string")
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (must be JSON-compatible; bytes/DataFrames are supported)
