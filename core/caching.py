...
class RedisCacheBackend(CacheBackend):
    ...
    def _get_signing_key(self) -> bytes:
        ...
        # Process-local key prevents unsigned/legacy payload loading by default.
        logger.warning(
    ...
    def _encode_value(self, value: Any) -> tuple[str, Any]:
        """Encode a cache value into a JSON-safe typed payload."""
    ...
    def _decode_value(self, payload_type: str, payload: Any) -> Any:
        """Decode a typed JSON-safe payload back into a Python value."""
        ...
        if payload_type == "pandas_dataframe":
            ...
            if not isinstance(frame_data, dict):
                raise ValueError("DataFrame data must be an object")
            restored = pd.read_json(StringIO(json.dumps(frame_data)), orient="split")
            if isinstance(dtypes, dict):
                usable = {col: dtype for col, dtype in dtypes.items() if col in restored.columns}
                if usable:
                    restored = restored.astype(usable)
            return restored
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            return self._decode_value(body["type"], body["payload"])
