...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for structured payloads
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value without unsafe code execution primitives."""
        if isinstance(value, bytes):
            ...
            # Validate JSON compatibility before storing the payload.
            json.dumps(value)
            envelope = {"v": 2, "type": "json", "payload": value}

        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize only supported safe cache payload types."""
        try:
    ...
    def _load_value(self, key: str, value: bytes) -> Any | None:
        """Deserialize value and drop invalid entries from Redis."""
    ...
    def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            if value is None:
                return None
            return self._load_value(key, value)
        except Exception as e:
