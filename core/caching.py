...
_CACHE_HMAC_ALG = "HMAC-SHA256"
_CACHE_PAYLOAD_JSON = "json"
_CACHE_PAYLOAD_BYTES = "bytes"
_CACHE_PAYLOAD_DATAFRAME = "pandas_dataframe"
_CACHE_PAYLOAD_TYPES = {
    _CACHE_PAYLOAD_JSON,
    _CACHE_PAYLOAD_BYTES,
    _CACHE_PAYLOAD_DATAFRAME,
}
...
class RedisCacheBackend(CacheBackend):
    ...
    def _encode_payload(self, value: Any) -> tuple[str, Any]:
        """Convert a cache value into a JSON-safe tagged payload."""
        if isinstance(value, bytes | bytearray):
            encoded = base64.b64encode(bytes(value)).decode("ascii")
            return _CACHE_PAYLOAD_BYTES, encoded
        if isinstance(value, pd.DataFrame):
            return _CACHE_PAYLOAD_DATAFRAME, value.to_json(orient="split", date_format="iso")
        try:
            json.dumps(value)
        ...
        return _CACHE_PAYLOAD_JSON, value

    def _decode_payload(self, payload_type: str, data: Any) -> Any:
        """Restore a tagged payload after envelope and signature checks."""
        if payload_type == _CACHE_PAYLOAD_JSON:
            return data
        if payload_type == _CACHE_PAYLOAD_BYTES:
            if not isinstance(data, str):
                raise ValueError("Bytes payload must be a base64 string")
            return base64.b64decode(data.encode("ascii"), validate=True)
        if payload_type == _CACHE_PAYLOAD_DATAFRAME:
            if not isinstance(data, str):
    ...
    def _signed_body(self, envelope: dict[str, Any]) -> bytes:
        """Canonical bytes used for HMAC over the cache envelope."""
        body = {
            "alg": envelope["alg"],
            "data": envelope["data"],
            "type": envelope["type"],
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value as a signed JSON envelope (never pickle)."""
        payload_type, data = self._encode_payload(value)
        envelope = {
        ...
        signature = hmac.new(
            self._signing_key, self._signed_body(envelope), hashlib.sha256
        ).hexdigest()
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            if not isinstance(envelope, dict):
                logger.warning("Rejected cache entry with non-object envelope")
                return None
            if (
                envelope.get("v") != _CACHE_ENVELOPE_VERSION
                or envelope.get("alg") != _CACHE_HMAC_ALG
                or envelope.get("type") not in _CACHE_PAYLOAD_TYPES
            ...
            return self._decode_payload(str(envelope["type"]), envelope["data"])
        ...
        try:
            ...
            if decoded is None:
                try:
                    ...
                    logger.warning(f"Failed to delete invalid cache entry {key}: {delete_error}")
            return decoded
        except Exception as e:
