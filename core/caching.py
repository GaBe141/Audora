...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value as a strict JSON envelope."""
        envelope = {"v": 2, "payload": self._to_safe_json(value)}
        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize supported JSON cache values without executing code."""
        try:
            ...
            return self._from_safe_json(envelope["payload"])
        except Exception as e:
            logger.error(f"Failed to deserialize cache entry: {e}")
            return None

    def _to_safe_json(self, value: Any) -> Any:
        """Convert supported Python values to JSON-compatible structures."""
        ...
        if isinstance(value, tuple):
            return {
                ...
                "items": [self._to_safe_json(item) for item in value],
            }

        if isinstance(value, list):
            return [self._to_safe_json(item) for item in value]

        if isinstance(value, dict):
            return {
                "__audora_type__": "dict",
                "items": [
                    [self._to_safe_json(key), self._to_safe_json(item)]
    ...
    def _from_safe_json(self, value: Any) -> Any:
        """Restore values serialized by _to_safe_json."""
        if value is None or isinstance(value, (bool, int, float, str)):
            return value

        if isinstance(value, list):
            return [self._from_safe_json(item) for item in value]
        ...
        if value_type == "tuple":
            ...
            if not isinstance(items, list):
                raise TypeError("Invalid tuple cache payload")
            return tuple(self._from_safe_json(item) for item in items)
        ...
        if value_type == "pandas_dataframe":
            ...
            data = value.get("data")
            if not isinstance(data, str):
                raise TypeError("Invalid dataframe cache payload")
            return pd.read_json(io.StringIO(data), orient="split")

        raise TypeError("Unsupported tagged cache payload")
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (must be JSON-compatible, bytes, tuple, or pandas DataFrame)
