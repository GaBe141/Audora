...
CACHE_TYPE_MARKER = "__audora_cache_type__"


def _to_cache_json(value: Any) -> Any:
    """Convert cache values into JSON-safe data without executable payloads."""
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    if isinstance(value, bytes):
        return {
            CACHE_TYPE_MARKER: "bytes",
            "data": base64.b64encode(value).decode("ascii"),
        }
    if isinstance(value, datetime):
        return {
            CACHE_TYPE_MARKER: "datetime",
            "data": value.isoformat(),
        }
    if isinstance(value, date):
        return {
            CACHE_TYPE_MARKER: "date",
            "data": value.isoformat(),
        }
    if isinstance(value, tuple):
        return {
            CACHE_TYPE_MARKER: "tuple",
            "items": [_to_cache_json(item) for item in value],
        }
    if isinstance(value, set):
        return {
            CACHE_TYPE_MARKER: "set",
            "items": [_to_cache_json(item) for item in sorted(value, key=repr)],
        }
    if isinstance(value, list):
        return [_to_cache_json(item) for item in value]
    if isinstance(value, dict):
        return {
            CACHE_TYPE_MARKER: "dict",
            "items": [
                [_to_cache_json(key), _to_cache_json(item)]
                for key, item in sorted(value.items(), key=lambda pair: repr(pair[0]))
            ],
        }
    if pd is not None and isinstance(value, pd.DataFrame):
        return {
            CACHE_TYPE_MARKER: "pandas.DataFrame",
            "orient": "split",
            "data": json.loads(value.to_json(orient="split", date_format="iso")),
        }
    if pd is not None and isinstance(value, pd.Series):
        return {
            CACHE_TYPE_MARKER: "pandas.Series",
            "name": _to_cache_json(value.name),
            "data": json.loads(value.to_json(orient="split", date_format="iso")),
        }

    raise TypeError(f"Value of type {type(value).__name__} is not safe for JSON caching")


def _from_cache_json(value: Any) -> Any:
    """Restore JSON-safe cache values produced by _to_cache_json."""
    if isinstance(value, list):
        return [_from_cache_json(item) for item in value]
    if not isinstance(value, dict) or CACHE_TYPE_MARKER not in value:
        return value

    cache_type = value[CACHE_TYPE_MARKER]
    if cache_type == "bytes":
    ...
    if cache_type == "date":
        return date.fromisoformat(value["data"])
    if cache_type == "tuple":
        return tuple(_from_cache_json(item) for item in value["items"])
    if cache_type == "set":
        return {_from_cache_json(item) for item in value["items"]}
    if cache_type == "dict":
        return {
            _from_cache_json(key): _from_cache_json(item)
            for key, item in value["items"]
        }
    if cache_type == "pandas.DataFrame":
        if pd is None:
            raise ValueError("Cannot restore pandas.DataFrame cache entry without pandas")
        data = value["data"]
        return pd.DataFrame(data=data["data"], columns=data["columns"], index=data["index"])
    if cache_type == "pandas.Series":
        if pd is None:
            raise ValueError("Cannot restore pandas.Series cache entry without pandas")
        data = value["data"]
        return pd.Series(data=data["data"], index=data["index"], name=_from_cache_json(value["name"]))

    raise ValueError(f"Unknown cache JSON type marker: {cache_type}")
...
class RedisCacheBackend(CacheBackend):
    ...
    def _get_signing_key(self) -> bytes:
        ...
        # Fallback to process-local random key to prevent unsigned payload reuse.
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            return _from_cache_json(json.loads(payload.decode("utf-8")))
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (must be JSON-cacheable)
