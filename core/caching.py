...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        if isinstance(value, bytes):
            ...
            # Ensure only JSON-native values are accepted. Avoid default=str so
            # unsupported objects are rejected instead of silently transformed.
            json.dumps(value)
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize cache value without executing cached content."""
        try:
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (must be JSON-serializable, bytes, or pandas DataFrame)
