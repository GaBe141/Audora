...
def _is_json_compatible(value: Any) -> bool:
    """Return True when value can be stored in a strict JSON cache envelope."""
...
def serialize_cache_value(value: Any, signing_key: bytes) -> bytes:
    ...
    Supports JSON-compatible values, bytes, and pandas DataFrames. Arbitrary
    objects are rejected so Redis never executes attacker-controlled payloads.
    """
    if isinstance(value, bytes):
        typed_payload: dict[str, Any] = {
            "v": _CACHE_FORMAT_VERSION,
            "t": "bytes",
            "d": base64.b64encode(value).decode("ascii"),
        }
    elif isinstance(value, pd.DataFrame):
        typed_payload = {
            "v": _CACHE_FORMAT_VERSION,
            "t": "pandas_dataframe",
            "d": value.to_json(orient="split", date_format="iso"),
        }
    elif _is_json_compatible(value):
        typed_payload = {"v": _CACHE_FORMAT_VERSION, "t": "json", "d": value}
    else:
        raise TypeError(f"Unsupported cache value type: {type(value)!r}")

    inner = json.dumps(typed_payload, separators=(",", ":"), ensure_ascii=True).encode("utf-8")
...
def deserialize_cache_value(raw: bytes, signing_key: bytes) -> Any | None:
    """Deserialize a signed JSON cache envelope, rejecting legacy/unsafe payloads."""
    try:
        envelope = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError):
        logger.warning("Rejected cache entry that is not valid JSON")
        return None

    if (
        not isinstance(envelope, dict)
        or envelope.get("v") != _CACHE_FORMAT_VERSION
        or envelope.get("alg") != _SIGNED_ALG
    ...
    try:
        ...
        logger.warning("Rejected cache entry with malformed payload encoding")
        return None
    ...
    try:
        typed_payload = json.loads(inner.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError):
        logger.warning("Rejected cache entry with invalid inner payload")
        return None

    if (
        not isinstance(typed_payload, dict)
        or typed_payload.get("v") != _CACHE_FORMAT_VERSION
        or "t" not in typed_payload
        or "d" not in typed_payload
    ):
        logger.warning("Rejected cache entry with invalid typed payload")
        return None

    payload_type = typed_payload["t"]
    data = typed_payload["d"]
    if payload_type == "json":
        if not _is_json_compatible(data):
            logger.warning("Rejected cache entry with non-JSON data")
            return None
        return data
    if payload_type == "bytes":
        ...
        try:
            ...
            logger.warning("Rejected cache entry with malformed bytes payload")
            return None
    if payload_type == "pandas_dataframe":
        ...
        try:
            ...
            logger.warning("Rejected cache entry with malformed dataframe payload: %s", e)
            return None
...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        Args:
            ...
            key_prefix: Prefix used to scope cache keys on shared Redis instances
        """
        if not REDIS_AVAILABLE:
            raise ImportError("Redis package not installed")

        if not key_prefix or any(ch in key_prefix for ch in "*?[]"):
            raise ValueError("Redis cache key_prefix must be a non-empty glob-safe string")
    ...
    def _get_signing_key(self) -> bytes:
        ...
        # Fallback to process-local random key so unsigned/legacy payloads cannot be loaded.
        logger.warning(
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize cache value only after signature and type verification."""
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        try:
            ...
            logger.error(f"Refused to cache unsupported value for key {key}: {e}")
        except Exception as e:
    ...
    def clear(self) -> None:
        ...
        Uses SCAN rather than FLUSHDB so a shared Redis database is not wiped.
        """
        try:
            ...
            if batch:
                self._client.delete(*batch)
            logger.debug("Cleared Redis cache keys matching %s", pattern)
        except Exception as e:
