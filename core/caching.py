...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for JSON payload bytes
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize supported cache values without unsafe code execution primitives."""
        envelope = {"v": REDIS_CACHE_PAYLOAD_VERSION, **self._encode_value(value)}
        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _encode_value(self, value: Any) -> dict[str, Any]:
        """Encode a supported value into a JSON-compatible cache payload."""
        ...
        if isinstance(value, pd.DataFrame):
            buffer = io.StringIO()
            value.to_json(buffer, orient="split", date_format="iso")
            return {"type": "pandas_dataframe", "value": buffer.getvalue()}

        raise TypeError(f"Unsupported Redis cache payload type: {type(value).__name__}")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize cache value from a strict JSON envelope."""
        try:
            envelope = json.loads(value.decode("utf-8"))
            if (
                not isinstance(envelope, dict)
                or envelope.get("v") != REDIS_CACHE_PAYLOAD_VERSION
            ...
            return self._decode_value(envelope["type"], envelope["value"])
        except Exception as e:
            logger.error(f"Failed to deserialize cache entry: {e}")
            return None

    def _decode_value(self, payload_type: Any, payload_value: Any) -> Any | None:
        """Decode a supported JSON cache payload."""
