...
class RedisCacheBackend(CacheBackend):
    ...
    _TYPE_JSON = "json"
    _TYPE_BYTES = "bytes"
    _TYPE_DATAFRAME = "pandas_dataframe"
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value without executable object deserialization."""
        if isinstance(value, bytes):
            envelope = {
                "v": self._ENVELOPE_VERSION,
                "type": self._TYPE_BYTES,
                "value": base64.b64encode(value).decode("ascii"),
            }
        elif isinstance(value, pd.DataFrame):
            envelope = {
                "v": self._ENVELOPE_VERSION,
                "type": self._TYPE_DATAFRAME,
            ...
            try:
                ...
                raise TypeError(
                    f"Redis cache only supports JSON-compatible values, bytes, and DataFrames; "
                    f"got {type(value).__name__}"
                ) from e
            envelope = {"v": self._ENVELOPE_VERSION, "type": self._TYPE_JSON, "value": value}

        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize cache value from a strict non-executable JSON envelope."""
        try:
            envelope = json.loads(value.decode("utf-8"))
            if (
                not isinstance(envelope, dict)
                or envelope.get("v") != self._ENVELOPE_VERSION
            ...
            value_type = envelope["type"]
            payload = envelope["value"]

            if value_type == self._TYPE_JSON:
                return payload

            if value_type == self._TYPE_BYTES:
                if not isinstance(payload, str):
                    logger.warning("Rejected bytes cache entry with non-string payload")
                    return None
                return base64.b64decode(payload.encode("ascii"), validate=True)

            if value_type == self._TYPE_DATAFRAME:
                if not isinstance(payload, str):
                    logger.warning("Rejected DataFrame cache entry with non-string payload")
                    return None
                return pd.read_json(StringIO(payload), orient="split")

            logger.warning("Rejected cache entry with unsupported payload type: %s", value_type)
            return None
        except Exception as e:
            logger.error(f"Failed to deserialize cache entry: {e}")
            return None

    def _delete_malformed_entry(self, key: str) -> None:
        """Best-effort cleanup for malformed or legacy Redis entries."""
        try:
            self._client.delete(key)
        except Exception as e:
            logger.warning(f"Failed to delete malformed cache entry {key}: {e}")

    def _is_valid_envelope(self, value: bytes) -> bool:
        """Return whether value looks like the current safe cache envelope."""
        try:
            envelope = json.loads(value.decode("utf-8"))
            if not isinstance(envelope, dict):
                return False
            return (
                envelope.get("v") == self._ENVELOPE_VERSION
                and envelope.get("type") in {self._TYPE_JSON, self._TYPE_BYTES, self._TYPE_DATAFRAME}
    ...
    def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            if value is None:
                return None
            if not self._is_valid_envelope(value):
                logger.warning("Rejected and removed malformed or legacy cache entry: %s", key)
                self._delete_malformed_entry(key)
                return None
