...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value to a safe, non-executable JSON envelope."""
    ...
    def _encode_value(self, value: Any) -> dict[str, Any]:
        """Convert supported cache values into JSON-compatible data."""
    ...
    def _decode_value(self, envelope: dict[str, Any]) -> Any | None:
        """Restore values from the safe JSON envelope."""
        ...
        if value_type == "bytes":
            ...
            if not isinstance(data, str):
                logger.warning("Rejected cache bytes entry with non-string data")
                return None
            return base64.b64decode(data.encode("ascii"), validate=True)

        if value_type == "pandas_dataframe":
            if not isinstance(data, str):
                logger.warning("Rejected cache DataFrame entry with non-string data")
                return None
            import pandas as pd
