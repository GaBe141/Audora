...
class RedisCacheBackend(CacheBackend):
    ...
    def _get_signing_key(self) -> bytes:
        ...
        # Fallback to a process-local random key so cache envelopes remain integrity-protected
        # even when no shared key is configured. This trades cross-process cache hit rate for safety.
        logger.warning(
    ...
    def _encode_supported_value(self, value: Any) -> dict[str, str]:
        """Encode supported value types into a JSON-serializable payload."""
        ...
        try:
            ...
            raise TypeError(
                ...
                "Use JSON-serializable values, bytes, or pandas DataFrame."
            ) from e

    def _decode_supported_value(self, payload: dict[str, Any]) -> Any:
        """Decode payload produced by _encode_supported_value."""
        value_type = payload.get("type")
        data = payload.get("data")

        if value_type == "json" and isinstance(data, str):
            return json.loads(data)

        if value_type == "bytes" and isinstance(data, str):
            return base64.b64decode(data.encode("ascii"), validate=True)

        if value_type == "pandas_dataframe" and isinstance(data, str):
            if pd is None:
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        encoded_payload = self._encode_supported_value(value)
        payload = json.dumps(encoded_payload, separators=(",", ":")).encode("utf-8")
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            if not isinstance(decoded, dict):
                ...
                return None
            return self._decode_supported_value(decoded)
    ...
    def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            if deserialized is None:
                # Remove malformed or tampered entries to avoid repeated parse work.
