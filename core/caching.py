...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Use bytes for controlled decoding
        )
    ...
    def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            if not isinstance(value, bytes):
                logger.warning(f"Unexpected Redis value type for key {key}: {type(value)!r}")
                return None

            try:
                return json.loads(value.decode("utf-8"))
            except (UnicodeDecodeError, json.JSONDecodeError):
                # Refuse to deserialize unknown/binary payloads to avoid unsafe execution
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        try:
            ...
            # Skip values that cannot be represented safely as JSON in Redis.
            logger.warning(f"Redis set skipped for key {key}: value is not JSON-serializable ({e})")
        except Exception as e:
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (Redis backend requires JSON-serializable values)
