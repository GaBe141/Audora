...
_CACHE_PAYLOAD_PREFIX = b"audora-cache:v1:"
...
class RedisCacheBackend(CacheBackend):
    ...
    def _load_signing_key(self) -> bytes:
        ...
        Signed cache payloads prevent unsafe deserialization of attacker-controlled
        Redis data. To share cache entries across processes, set AUDORA_CACHE_HMAC_KEY.
        """
        env_key = os.getenv("AUDORA_CACHE_HMAC_KEY", "")
        if env_key:
            return env_key.encode("utf-8")

        logger.warning(
            "AUDORA_CACHE_HMAC_KEY not set; using ephemeral cache signing key. "
            "Redis cache entries will only be readable by this process."
        )
        return secrets.token_bytes(32)

    def _serialize_value(self, value: Any) -> bytes:
        """Serialize and sign cache value before storing it."""
        payload = pickle.dumps(value, protocol=pickle.HIGHEST_PROTOCOL)
        signature = hmac.new(self._signing_key, payload, hashlib.sha256).hexdigest().encode("ascii")
        return _CACHE_PAYLOAD_PREFIX + signature + b":" + payload

    def _deserialize_value(self, raw: bytes) -> Any | None:
        """Verify signature and deserialize a cache value safely."""
        if not raw.startswith(_CACHE_PAYLOAD_PREFIX):
            logger.warning("Rejected unsigned or legacy Redis cache payload")
            return None

        signed_payload = raw[len(_CACHE_PAYLOAD_PREFIX) :]
        try:
            signature_hex, payload = signed_payload.split(b":", 1)
        except ValueError:
            logger.warning("Rejected malformed Redis cache payload")
            return None
        ...
        if not hmac.compare_digest(signature_hex, expected_signature):
            logger.warning("Rejected Redis cache payload with invalid signature")
            return None
