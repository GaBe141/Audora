...
class RedisCacheBackend(CacheBackend):
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize cache value from the strict JSON envelope."""
        try:
    ...
    def _to_safe_envelope(self, value: Any) -> dict[str, Any]:
        """Convert supported cache values to a non-executable JSON envelope."""
        ...
        if self._is_json_compatible(value):
            return {"v": 1, "type": "json", "value": value}
    ...
    def _from_safe_envelope(self, envelope: dict[str, Any]) -> Any | None:
        """Restore a supported cache value from a validated JSON envelope."""
    ...
    def _is_json_compatible(self, value: Any) -> bool:
        """Return True when value can be represented as strict JSON."""
