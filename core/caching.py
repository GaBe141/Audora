...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for bytes-safe payloads
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize supported cache values without executable object formats."""
        ...
        if isinstance(value, bytes):
            ...
            # Validate JSON compatibility up front so Redis never stores arbitrary objects.
            envelope.update({"type": "json", "payload": json.loads(json.dumps(value))})

        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize cache values from the strict JSON cache envelope."""
        try:
    ...
    def _is_pandas_dataframe(self, value: Any) -> bool:
        """Return True for pandas DataFrame values without making pandas mandatory."""
        try:
            import pandas as pd
        except ImportError:
            return False
    ...
    def _deserialize_dataframe(self, payload: Any) -> Any | None:
        """Rebuild a pandas DataFrame from a split-orientation JSON payload."""
        try:
            import pandas as pd
