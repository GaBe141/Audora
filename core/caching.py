...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value using safe, typed JSON payloads."""
        envelope = self._build_serialization_envelope(value)
        return json.dumps(envelope, separators=(",", ":"), allow_nan=False).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize a cache value without executing attacker-controlled code."""
        try:
            ...
            if payload_type == "pandas_dataframe":
                return self._deserialize_dataframe(payload)
    ...
    def _build_serialization_envelope(self, value: Any) -> dict[str, Any]:
        """Build a versioned JSON envelope for supported cache value types."""
        if isinstance(value, bytes):
            return {
                "v": 1,
                "type": "bytes",
                "payload": b64encode(value).decode("ascii"),
            }
    ...
    def _deserialize_dataframe(self, payload: Any) -> Any | None:
        """Deserialize a pandas DataFrame from a split-orient JSON payload."""
        if not (
        ...
        try:
            ...
            logger.error(f"Failed to deserialize DataFrame cache entry: {e}")
            return None

    def _is_pandas_dataframe(self, value: Any) -> bool:
        """Return True when value is a pandas DataFrame without requiring pandas at import time."""
        return (
    ...
    def _is_json_compatible(self, value: Any) -> bool:
        """Return True when value can round-trip through strict JSON without coercion."""
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache in Redis must be JSON-compatible, bytes, or a pandas DataFrame
