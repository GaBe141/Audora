...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._allow_pickle = self._allow_pickle_cache_payloads()
        if self._allow_pickle:
            logger.warning(
                "AUDORA_CACHE_ALLOW_PICKLE is enabled. Pickle in Redis cache can be unsafe; "
                "keep this disabled unless you fully trust all cache writers."
            )
    ...
    def _allow_pickle_cache_payloads(self) -> bool:
        """Whether Redis cache should allow pickle payloads (unsafe by default)."""
        ...
        try:
            ...
            if not self._allow_pickle:
                raise ValueError(
                    ...
                    "Set AUDORA_CACHE_ALLOW_PICKLE=true only if you explicitly trust cache inputs."
                ) from e
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            encoding = envelope.get("enc", "pickle" if envelope.get("v") == 1 else "json")
            if encoding == "json":
                return json.loads(payload.decode("utf-8"))

            if encoding == "pickle":
                if not self._allow_pickle:
                    logger.warning("Rejected pickle cache payload while pickle support is disabled")
                    return None
                return pickle.loads(payload)

            logger.warning(f"Rejected cache entry with unknown encoding: {encoding}")
            return None
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (JSON-serializable by default for Redis backend)
