...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(value: Any) -> bytes | None:
        """Serialize cache value using non-executable, typed JSON payloads."""
        try:
            if isinstance(value, bytes | bytearray):
            ...
            elif PANDAS_AVAILABLE and pd is not None and isinstance(value, pd.DataFrame):
                envelope = {
                    "v": 2,
                ...
                # Validate that the value can round-trip through JSON before storing it.
                json.dumps(value, allow_nan=False)
                envelope = {"v": 2, "type": "json", "payload": value}

            return json.dumps(envelope, separators=(",", ":")).encode("utf-8")
        except (TypeError, ValueError) as e:
            logger.warning(f"Unsupported cache value type for Redis serialization: {e}")
            return None
