...
def _encode_cache_payload(value: Any) -> tuple[str, Any]:
    ...
    if pd is not None and isinstance(value, pd.DataFrame):
        return "pandas_dataframe", json.loads(value.to_json(orient="split", date_format="iso"))

    # Reject values that cannot be round-tripped through strict JSON.
    json.dumps(value)
    return "json", value


def _decode_cache_payload(kind: str, payload: Any) -> Any:
    ...
    if kind == "pandas_dataframe":
        if pd is None:
            raise ValueError("pandas is required to deserialize DataFrame cache entries")
...
def serialize_cache_value(value: Any, signing_key: bytes) -> bytes:
    ...
    Pickle is intentionally not used: a compromised Redis host or leaked signing
    key must not be able to execute attacker-controlled bytecode.
    """
    kind, payload = _encode_cache_payload(value)
    body = json.dumps({"kind": kind, "payload": payload}, separators=(",", ":"), sort_keys=True)
...
def deserialize_cache_value(raw: bytes, signing_key: bytes) -> Any:
    ...
    Rejects legacy pickle payloads, unsigned blobs, and tampered signatures.
    """
    try:
        envelope = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("Rejected cache entry with invalid serialization envelope") from exc
    if (
        ...
        raise ValueError("Rejected cache entry with invalid serialization envelope")

    body = json.dumps(
        {"kind": envelope["kind"], "payload": envelope["payload"]},
        separators=(",", ":"),
        sort_keys=True,
    )
    expected_sig = hmac.new(signing_key, body.encode("utf-8"), hashlib.sha256).hexdigest()
    if not hmac.compare_digest(envelope["sig"], expected_sig):
        raise ValueError("Rejected cache entry with invalid signature")

    return _decode_cache_payload(envelope["kind"], envelope["payload"])
...
class RedisCacheBackend(CacheBackend):
    ...
    def _get_signing_key(self) -> bytes:
        ...
        # Fallback to a process-local random key so unsigned/legacy payloads cannot be loaded.
        logger.warning(
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value as a signed JSON envelope without pickle."""
        return serialize_cache_value(value, self._signing_key)

    def _deserialize(self, value: bytes) -> Any:
        """Deserialize a signed JSON cache envelope. Raises ValueError on invalid payloads."""
    ...
    def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            try:
                ...
                logger.debug("Failed to delete invalid Redis cache entry for key %s", key)
            return None
