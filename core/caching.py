...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for cache payload envelopes
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value into a safe, non-executable JSON envelope."""
        envelope = {"v": 2, **self._encode_value(value)}
    ...
    def _encode_value(self, value: Any) -> dict[str, Any]:
        ...
        try:
            ...
            raise TypeError(
                "Redis cache only supports JSON-serializable values, bytes, and pandas DataFrames"
            ) from e

        return {"type": "json", "payload": value}

    def _decode_value(self, envelope: dict[str, Any]) -> Any | None:
        """Decode a validated cache envelope."""
