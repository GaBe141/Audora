...
class RedisCacheBackend(CacheBackend):
    ...
    def _get_signing_key(self) -> bytes:
        ...
        # Fallback to process-local random key so unsigned/forged cache payloads are rejected.
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value using non-executable JSON encoding."""
        encoded_value = self._encode_value(value)
        payload = json.dumps(encoded_value, separators=(",", ":")).encode("utf-8")
    ...
    def _encode_value(self, value: Any) -> Any:
        """Encode Python values into a JSON-safe representation."""
        ...
        if isinstance(value, dict):
            ...
            for key, item in value.items():
                ...
                encoded_dict[key] = self._encode_value(item)
            return encoded_dict
        ...
        if isinstance(value, date):
            return {self._TYPE_KEY: "date", "value": value.isoformat()}
        ...
        if PANDAS_AVAILABLE:
            if isinstance(value, pd.DataFrame):
                return {
                    self._TYPE_KEY: "pandas.DataFrame",
                    "orient": "split",
                    "data": value.to_json(orient="split", date_format="iso"),
                }
            if isinstance(value, pd.Series):
                return {
                    self._TYPE_KEY: "pandas.Series",
                    "orient": "split",
                    "data": value.to_json(orient="split", date_format="iso"),
                }
    ...
    def _decode_value(self, value: Any) -> Any:
        """Decode values previously encoded with _encode_value."""
        if value is None or isinstance(value, (str, bool, int, float)):
            return value
        ...
        if isinstance(value, dict):
            encoded_type = value.get(self._TYPE_KEY)
            if encoded_type is None:
                return {key: self._decode_value(item) for key, item in value.items()}

            if encoded_type == "bytes":
                payload = value.get("b64", "")
                if not isinstance(payload, str):
                    raise ValueError("Invalid bytes payload in cache entry")
                return base64.b64decode(payload.encode("ascii"), validate=True)

            if encoded_type == "tuple":
                return tuple(self._decode_value(item) for item in value.get("items", []))

            if encoded_type == "set":
                return set(self._decode_value(item) for item in value.get("items", []))
            ...
            if encoded_type == "ndarray":
                ...
                return np.array(value.get("items", []), dtype=value.get("dtype"))

            if encoded_type == "pandas.DataFrame":
                ...
                return pd.read_json(StringIO(data), orient=orient)

            if encoded_type == "pandas.Series":
                if not PANDAS_AVAILABLE:
                    raise ValueError("Pandas is required to deserialize Series cache entries")
                data = value.get("data", "")
                orient = value.get("orient", "split")
                if not isinstance(data, str):
                    raise ValueError("Invalid Series payload in cache entry")
                return pd.read_json(StringIO(data), orient=orient, typ="series")

            raise ValueError(f"Unknown cache payload type: {encoded_type}")

        raise TypeError(f"Invalid cache payload container: {type(value).__name__}")
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            if envelope.get("v") == 1:
                # Explicitly reject old pickle-based format to eliminate RCE risk.
