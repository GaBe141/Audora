...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep explicit byte payload handling
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        if isinstance(value, bytes):
            ...
            try:
                ...
                raise TypeError(
                    f"Redis cache does not support values of type {type(value).__name__}"
                ) from e
            envelope = {"v": 1, "type": "json", "data": value}

        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize versioned safe cache payloads."""
        try:
            ...
            if value_type == "bytes":
                if not isinstance(data, str):
                    logger.warning("Rejected bytes cache entry with non-string data")
                    return None
                return base64.b64decode(data.encode("ascii"), validate=True)
            if not isinstance(data, str):
                logger.warning("Rejected DataFrame cache entry with non-string data")
                return None
