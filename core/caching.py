...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for serialized JSON payloads
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        Redis is commonly shared infrastructure, so cache entries must be treated
        as untrusted. Only explicit, non-executable payload types are accepted.
        """
        value_type = "json"
        payload: Any
        ...
        if isinstance(value, bytes):
            ...
            if pd is not None and isinstance(value, pd.DataFrame):
                value_type = "pandas_dataframe"
                payload = value.to_json(orient="split", date_format="iso")
            else:
                # Round-trip through strict JSON so unsupported objects are rejected
                # instead of being coerced with repr/default hooks.
                payload = json.loads(json.dumps(value, allow_nan=False))

        envelope = {"v": 1, "type": value_type, "payload": payload}
        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize cache values from the safe, typed JSON envelope."""
        try:
