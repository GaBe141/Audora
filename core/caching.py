...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value into a safe JSON envelope.

        Redis is often shared infrastructure, so cache entries must never require
        unsafe deserialization. Unsupported values are skipped instead of pickled.
        """
        envelope = self._to_safe_envelope(value)
        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize a cache value from a safe JSON envelope."""
        try:
            ...
            return self._from_safe_envelope(envelope)
        except Exception as e:
            logger.error(f"Failed to deserialize cache entry: {e}")
            return None

    def _to_safe_envelope(self, value: Any) -> dict[str, Any]:
        """Convert supported values to a versioned JSON-compatible envelope."""
        if self._is_json_compatible(value):
            return {"v": 2, "type": "json", "value": value}
    ...
    def _from_safe_envelope(self, envelope: dict[str, Any]) -> Any | None:
        """Rehydrate supported values from a versioned cache envelope."""
        payload_type = envelope["type"]
        payload_value = envelope["value"]

        if payload_type == "json":
            if self._is_json_compatible(payload_value):
                return payload_value
            logger.warning("Rejected cache entry with invalid JSON payload")
            return None

        if payload_type == "bytes":
            if envelope.get("encoding") != "hex" or not isinstance(payload_value, str):
                logger.warning("Rejected cache entry with invalid bytes payload")
                return None
            return bytes.fromhex(payload_value)

        if payload_type == "pandas_dataframe":
            if envelope.get("orient") != "split" or not isinstance(payload_value, str):
        ...
        logger.warning(f"Rejected cache entry with unsupported type: {payload_type}")
        return None

    def _is_json_compatible(self, value: Any) -> bool:
        """Return True when a value can safely round-trip through JSON."""
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache in Redis (JSON-compatible, bytes, or pandas DataFrame)
