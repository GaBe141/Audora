...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for signed envelopes
        )
    ...
    def _get_signing_key(self) -> bytes:
        ...
        # Fallback to a process-local random key so unsigned/tampered cache entries
        # are never accepted if no explicit signing key is configured.
        logger.warning(
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        try:
            ...
            raise ValueError(
                "Redis cache only supports JSON-serializable values"
            ) from exc
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            if not isinstance(payload_json, str):
                logger.warning("Rejected cache entry with non-string JSON payload")
                return None
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (JSON-serializable when using Redis backend)
