...
class RedisCacheBackend(CacheBackend):
    ...
    def _get_signing_key(self) -> bytes:
        ...
        # Fallback to process-local random key to prevent accepting tampered cache data.
    ...
    def _to_json_safe(self, value: Any) -> Any:
        """Convert supported cache values into explicit JSON-safe types."""
        ...
        if value_module.startswith("pandas") and value_name == "DataFrame":
            return {
                "__audora_type__": "pandas_dataframe",
                "data": value.to_json(orient="split", date_format="iso"),
            }
        if value_module.startswith("pandas") and value_name == "Series":
            return {
                "__audora_type__": "pandas_series",
                "data": value.to_json(orient="split", date_format="iso"),
            }

        raise TypeError(
            f"Unsupported cache value type for Redis JSON serialization: {type(value)!r}"
        )

    def _from_json_safe(self, value: Any) -> Any:
        """Restore values encoded by _to_json_safe without executing code."""
        if value is None or isinstance(value, str | int | float | bool):
            return value
        ...
        if not isinstance(value, dict):
            raise TypeError(f"Unsupported encoded cache value: {type(value)!r}")

        value_type = value.get("__audora_type__")
        if value_type == "bytes":
            return base64.b64decode(value["data"].encode("ascii"), validate=True)
        if value_type == "tuple":
            return tuple(self._from_json_safe(item) for item in value["items"])
        if value_type == "set":
            return {self._from_json_safe(item) for item in value["items"]}
        if value_type == "dict":
            return {
        ...
        if value_type == "pandas_series":
            ...
            return pd.read_json(StringIO(value["data"]), orient="split", typ="series")

        raise TypeError("Rejected cache entry with unknown encoded type")

    def _serialize(self, value: Any) -> bytes:
        """Serialize supported cache values as signed JSON."""
        payload = json.dumps(
            {
                "v": 2,
                "format": "json",
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        ...
        try:
            ...
            if (
                not isinstance(payload_doc, dict)
                or payload_doc.get("v") != 2
                or payload_doc.get("format") != "json"
                or "data" not in payload_doc
            ):
                logger.warning("Rejected cache entry with invalid JSON payload")
                return None

            return self._from_json_safe(payload_doc["data"])
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (must be JSON-serializable or a supported safe type)
