...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for serialized cache payloads
        )
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize only explicit safe cache payload types."""
        try:
            ...
            if payload_type == "bytes" and isinstance(payload_value, str):
                return base64.b64decode(payload_value.encode("ascii"), validate=True)
            if payload_type == "pandas_dataframe" and isinstance(payload_value, str):
