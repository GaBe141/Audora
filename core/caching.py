...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for safe JSON envelopes
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value to a safe, non-executable wire format."""
        envelope = self._to_safe_envelope(value)
        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any:
        """Deserialize a cache value from the safe Redis wire format."""
        try:
            ...
            if payload_type == "pandas_dataframe":
                return self._dataframe_from_payload(payload)
            ...
            raise ValueError(f"failed to deserialize cache entry: {e}") from e

    def _to_safe_envelope(self, value: Any) -> dict[str, Any]:
        """Build a versioned cache envelope for JSON-compatible values."""
        if self._is_json_compatible(value):
            return {"v": 1, "type": "json", "payload": value}
        ...
        dataframe_payload = self._dataframe_to_payload(value)
        if dataframe_payload is not None:
            return {"v": 1, "type": "pandas_dataframe", "payload": dataframe_payload}

        raise TypeError(f"Unsupported cache value type: {type(value).__name__}")

    def _is_json_compatible(self, value: Any) -> bool:
        """Return True when value can round-trip through strict JSON unchanged."""
    ...
    def _dataframe_to_payload(self, value: Any) -> dict[str, Any] | None:
        """Convert a pandas DataFrame into JSON-compatible split-orient data."""
        try:
            import pandas as pd
        except ImportError:
            return None
    ...
    def _dataframe_from_payload(self, payload: Any) -> Any:
        """Rehydrate a pandas DataFrame from split-orient cache data."""
        ...
        try:
            ...
            raise ValueError("pandas is required to deserialize dataframe payloads") from e

        required_keys = {"columns", "index", "data"}
        if not required_keys.issubset(payload):
            raise ValueError("dataframe payload is missing required split-orient keys")
