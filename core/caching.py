...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for explicit JSON payload handling
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        Redis can be shared infrastructure, so cache entries must be treated as
        untrusted input. Keep the wire format to explicit JSON envelopes only.
        """
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize explicit JSON cache envelopes."""
        try:
            ...
            if payload_type == "dataframe":
                if not PANDAS_AVAILABLE or not isinstance(payload, dict):
                    logger.warning("Rejected dataframe cache entry with invalid payload")
                    return None
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (must be JSON-serializable, bytes, or a DataFrame)
