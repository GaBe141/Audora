...
class RedisCacheBackend(CacheBackend):
    ...
    def _to_json_compatible(self, value: Any) -> Any:
        """Convert cache values to a strict JSON-compatible shape."""
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value without unsafe object deserialization."""
        if isinstance(value, bytes):
            payload_type = "bytes"
            payload = base64.b64encode(value).decode("ascii")
        elif value.__class__.__module__.startswith("pandas") and value.__class__.__name__ == "DataFrame":
            payload_type = "pandas_dataframe"
            payload = value.to_json(orient="split", date_format="iso")
        else:
            payload_type = "json"
            payload = self._to_json_compatible(value)
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize only the supported safe cache payload formats."""
        try:
