...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value into a typed JSON envelope.

        Redis cache entries may be modified outside this process, so values must
        never require executable deserialization such as pickle.
        """
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize typed JSON cache values without executing payload code."""
        try:
