...
class RedisCacheBackend(CacheBackend):
    ...
    def _get_signing_key(self) -> bytes:
        ...
        # Fallback to process-local random key for tamper protection.
        # This keeps cache integrity safe by default with reduced cross-process hit rate.
        logger.warning(
    ...
    def _encode_value(self, value: Any) -> dict[str, Any]:
        """Encode cache value into a JSON-safe structure."""
        ...
        try:
            # Validate this value is JSON-serializable before writing to Redis.
            json.dumps(value)
    ...
    def _decode_value(self, encoded_value: Any) -> Any | None:
        """Decode a JSON-safe cache structure back to an in-memory object."""
        if not isinstance(encoded_value, dict):
            logger.warning("Rejected cache entry with non-dict encoded payload")
            return None

        value_type = encoded_value.get("type")
        if value_type == "json":
            return encoded_value.get("value")
        ...
        if value_type == "pandas_dataframe":
            ...
            frame_json = encoded_value.get("value")
            if not isinstance(frame_json, str):
                logger.warning("Rejected pandas cache payload with invalid data type")
                return None
            return pd.read_json(StringIO(frame_json), orient="split")

        logger.warning(f"Rejected cache entry with unsupported encoded type: {value_type}")
        return None
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        try:
            ...
            if serialized is None:
                logger.warning(f"Skipping Redis cache set for key {key}: unsupported value type")
                return
            if ttl:
...
class CacheManager:
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> None:
        ...
        Args:
            key: Cache key
            value: Value to cache (JSON-serializable or a pandas DataFrame for Redis backend)
