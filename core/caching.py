...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value to a JSON-only envelope."""
        envelope = {"v": 2, "format": "json", "value": self._to_json_compatible(value)}
        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize JSON cache values; legacy pickle envelopes are rejected."""
        try:
            ...
            return self._from_json_compatible(envelope["value"])
        except Exception as e:
            logger.error(f"Failed to deserialize cache entry: {e}")
            return None

    def _to_json_compatible(self, value: Any) -> Any:
        """Convert supported cache values into tagged JSON-compatible structures."""
        ...
        if isinstance(value, tuple):
            return {
                ...
                "items": [self._to_json_compatible(item) for item in value],
            }
        ...
        if isinstance(value, dict):
            ...
            for key, item in value.items():
                ...
                encoded[key] = self._to_json_compatible(item)
            return encoded

        item = getattr(value, "item", None)
        if callable(item):
            return self._to_json_compatible(item())

        raise TypeError(f"Unsupported Redis cache value type: {type(value).__name__}")

    def _from_json_compatible(self, value: Any) -> Any:
        """Restore values serialized by _to_json_compatible."""
        if isinstance(value, list):
            return [self._from_json_compatible(item) for item in value]
        if not isinstance(value, dict):
            return value

        cache_type = value.get("__audora_cache_type__")
        if cache_type == "bytes":
            data = value.get("data")
            if not isinstance(data, str):
                raise ValueError("Invalid cached bytes payload")
            return base64.b64decode(data.encode("ascii"), validate=True)
        if cache_type == "dataframe":
            if value.get("orient") != "split":
                raise ValueError("Unsupported cached DataFrame orientation")
            return pd.read_json(StringIO(json.dumps(value["data"])), orient="split")
        if cache_type == "tuple":
            items = value.get("items")
            if not isinstance(items, list):
                raise ValueError("Invalid cached tuple payload")
            return tuple(self._from_json_compatible(item) for item in items)
        if cache_type:
            raise ValueError(f"Unsupported cached value type: {cache_type}")

        return {key: self._from_json_compatible(item) for key, item in value.items()}
