...
_SUPPORTED_KINDS = {"json", "bytes", "pandas_dataframe"}


def _json_default(obj: Any) -> Any:
    """Convert common non-JSON types without using pickle."""
...
class RedisCacheBackend(CacheBackend):
    """Redis cache backend with connection pooling and safe JSON envelopes."""
    ...
    def _mac_message(self, kind: str, data: str) -> bytes:
        """Canonical message used for HMAC verification."""
        body = {
            "alg": "HMAC-SHA256",
            "data": data,
            "kind": kind,
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        if isinstance(value, pd.DataFrame):
            ...
            kind = "json"
            data = json.dumps(value, separators=(",", ":"), sort_keys=True, default=_json_default)

        signature = hmac.new(
            self._signing_key, self._mac_message(kind, data), hashlib.sha256
        ).hexdigest()
        envelope = {
            "v": _CACHE_ENVELOPE_VERSION,
            "alg": "HMAC-SHA256",
    ...
    def _deserialize(self, value: bytes) -> Any:
        """Deserialize a signed JSON envelope. Returns _MISSING on rejection."""
        try:
            envelope = json.loads(value.decode("utf-8"))
            if not isinstance(envelope, dict):
                logger.warning("Rejected cache entry that is not a JSON object")
                return _MISSING
            ...
            if (
                ...
                or kind not in _SUPPORTED_KINDS
            ...
            logger.warning("Rejected cache entry with unsupported kind: %s", kind)
            return _MISSING
    ...
    def clear(self) -> None:
        """Clear keys for this cache prefix only (never FLUSHDB)."""
        try:
