...
class RedisCacheBackend(CacheBackend):
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value without unsafe code-loading formats."""
        if isinstance(value, pd.DataFrame):
            envelope = {
                "v": 1,
                "kind": "pandas_dataframe",
                "payload": value.to_json(orient="split", date_format="iso", default_handler=str),
            }
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize cache value from explicit safe formats only."""
        try:
            ...
            if kind == "bytes":
                if not isinstance(payload, str):
                    logger.warning("Rejected cache bytes entry with non-string payload")
                    return None
                return base64.b64decode(payload.encode("ascii"), validate=True)

            if kind == "pandas_dataframe":
                if not isinstance(payload, str):
                    logger.warning("Rejected cache DataFrame entry with non-string payload")
                    return None
                return pd.read_json(StringIO(payload), orient="split")

            logger.warning("Rejected cache entry with unsupported serialization kind: %s", kind)
            return None
