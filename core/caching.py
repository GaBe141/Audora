...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for versioned payloads
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        if isinstance(value, bytes):
            payload = {"type": "bytes", "data": value.hex()}
            ...
            # Restrict Redis cache values to JSON-compatible primitives and containers.
            json.dumps(value)
    ...
    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize cache value from safe, non-executable payload types."""
        try:
            ...
            if payload_type == "bytes":
                ...
                if not isinstance(data, str):
                    logger.warning("Rejected bytes cache entry with invalid data")
                    return None
            ...
            if payload_type == "pandas_dataframe":
                ...
                data = payload.get("data")
                if not isinstance(data, str):
                    logger.warning("Rejected DataFrame cache entry with invalid data")
                    return None
