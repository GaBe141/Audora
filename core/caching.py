...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(
        ...
        self._pool = ConnectionPool(
            ...
            decode_responses=False,  # Keep binary mode for versioned cache payloads
        )
    ...
    def _serialize(self, value: Any) -> bytes:
        """Serialize cache value without executable object formats."""
        envelope = self._build_safe_envelope(value)
        return json.dumps(envelope, separators=(",", ":")).encode("utf-8")

    def _deserialize(self, value: bytes) -> Any | None:
        """Deserialize versioned cache values without executing payload code."""
        try:
            ...
            return self._load_safe_envelope(envelope)
        except Exception as e:
            logger.error(f"Failed to deserialize cache entry: {e}")
            return None

    def _build_safe_envelope(self, value: Any) -> dict[str, Any]:
        """Build a JSON-compatible cache envelope for supported value types."""
        ...
        if value.__class__.__module__.startswith("pandas") and value.__class__.__name__ == "DataFrame":
            ...
            return {"v": 2, "type": "pandas_dataframe", "payload": frame_payload}

        try:
            json.dumps(value, allow_nan=False)
        except (TypeError, ValueError) as e:
            raise TypeError(
                "Redis cache values must be JSON-compatible, bytes, or pandas DataFrames"
            ) from e

        return {"v": 2, "type": "json", "payload": value}

    def _load_safe_envelope(self, envelope: dict[str, Any]) -> Any | None:
        """Load a supported value type from a validated cache envelope."""
        ...
        if payload_type == "pandas_dataframe":
            if not isinstance(payload, dict):
                logger.warning("Rejected DataFrame cache entry with non-object payload")
                return None
            try:
                import pandas as pd
            except ImportError:
